...
import game.core.map.MapConfig;
import game.core.net.thread.ServerThread;
import game.core.net.thread.config.ThreadConfig;
import game.core.net.thread.loader.ThreadConfigXmlLoader;
...
public abstract class MapServer implements Runnable {
    ...
    protected MapServer(String name, long zoneId, int zoneModelId, List<MapConfig> mapConfigs, String threadConfig) {
        ...
        if (threadConfig != null) {
            ...
            this.threadConfigs = new ThreadConfigXmlLoader().load(defaultThreadConfigFile);
        }
    ...
    public void addCommand(ICommand command) {
        ...
        //寻找处理队列
        ServerThread thread = (ServerThread) this.thread_pool.get(BaseServer.DEFAULT_MAIN_THREAD);
        if (thread != null) {
            //添加命令
