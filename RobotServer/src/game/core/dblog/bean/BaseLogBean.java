...
public abstract class BaseLogBean
{
    ...
    //字段DDL信息 用于建表
    private static ConcurrentHashMap<Class<? extends BaseLogBean>, HashSet<MetaData>> metadataset = new ConcurrentHashMap<>();
    //添加索引信息
    private static ConcurrentHashMap<Class<? extends BaseLogBean>, HashSet<MetaData>> metadatasetindex = new ConcurrentHashMap<>();
        ...
        if (!metadataset.containsKey(getClass()))
        {
            ...
            for (Method method : declaredMethods)
            {
                ...
                if (annotation != null)
                {
                    ...
                    else if (!annotation.index().equals("0"))
                    {                  //索引字段为0时不设置
...
//		case UNROLL:
    ...
    public String buildCreateTableSql(long time)
    {
        ...
        DDL.append(") ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 ");
