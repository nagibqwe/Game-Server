...
public class MapsConfigManager {
    ...
    //获取MapsConfigManager的实例对象
    public static MapsConfigManager getInstance() {
        return Singleton.INSTANCE.getProcessor();
    }

    public ConcurrentHashMap<Integer, ByteMapCfg> getMapCfgStore() {
        return mapCfgStore;
    }
    ...
    public boolean loadMaps() {
        ...
        for (Cfg_Mapsetting_Bean mapsetting : CfgManager.getCfg_Mapsetting_Container().getValuees()) {
            ...
            if (!readMapInfo(mapsetting.getMap_info(), map)) { //加载地图各种配置
                if (mapsetting.getType() == MapDefine.WorldMap) {
    ...
    //加载地图配置
    public boolean test_loadMaps() {

        //test code
        ByteMapCfg map = new ByteMapCfg();
        map.setMapID(9527);
        readMap("map052_PathGrid.bytes", map); //加载地图纹理
        readMapInfo("map052_MapInfo.bytes", map); //加载地图各种配置
        mapCfgStore.put(9527, map);
        //TODO 根据文件 mapsetting 的两个字段加载地图配置
    ...
    public void reloadJumpTran() {
        ...
        try {
            ...
            for (int i = 0; i < count; i++) {
                ...
                log.error("跳跃传送ID=" + transID + " curPos=" + vec);
            }
            bos.close();
    ...
    public ByteMapCfg reloadMap(int mapModelID) {
        ...
        if (!readMap(config.getMap_grid(), map)) { //加载地图纹理
            return null;
        }
        if (!readMapInfo(config.getMap_info(), map)) { //加载地图各种配置
            return null;
        }
        mapCfgStore.put(map.getMapID(), map);
    ...
    private boolean readMap(String filename, ByteMapCfg map) {
        ...
        try {
            ...
            log.error("加载阻挡信息 mapID: " + map.getMapID() + " 文件内容：" + (MapFileHead + filename));
            /////////////////////////////读取基本信息/////////////////////
            int len = bis.readInt();
            String mapName = bis.readString(len);  //地图名
...
//            /////////////////////////////////////////////////////////////////////
//            //检测版本
//            if (version != 1) //TODO log and break
//            {
//                log.error("mapcfg error version");
//            }
//
//            if (!path.equals("PATH")) //文件不是PathGrid文件，策划配置填错了
//            //TODO log  and break
//            {
//                log.error("mapcfg error PATH");
//            }
////
//            //检测文件大小
//            if (fileSize != file.length()) //TODO log and break
//            {
//                log.error("mapcfg error filesize");
//            }
//
//            //检测偏移量 is overflow
...
//
//            if (offset1 > fileSize || offset2 > fileSize || offset3 > fileSize) //TODO  log and break
//            {
//                log.error("mapcfg error offset12345");
//            }
//            //检测阻挡数据
//            int totalCellCount = colCellCount * rowCellCount;
//            if (blockInfoLen != totalCellCount || heightInfoLen / 4 != totalCellCount) //TODO log and break
//            {
//                log.error("mapcfg error blockinfo");
//            }
            ...
            log.error(mapName + " 读取阻档数据：" + (System.currentTimeMillis() - nano) + "毫秒");
            ...
            log.error(mapName + " 加载阻档数据用时：" + (System.currentTimeMillis() - nano) + "毫秒, 条数：" + " x=" + rowCellCount + ",y=" + colCellCount + " =" + blocks.length * blocks[0].length);
    ...
    private boolean readMapInfo(String filename, ByteMapCfg map) {
        ...
        try {
            ...
            //检测 文件类型
            if (!scen.equals("SCEN")) //TODO log and break
            {
            ...
            if (Offset1 > fileSize || Offset2 > fileSize
                    ...
                    || Offset7 > fileSize || Offset8 > fileSize) //TODO log and break
            {
                log.error("mapinfocfg error version");
            }
            ...
            if (!eof.equals("EOF")) //TODO log and break
            {
