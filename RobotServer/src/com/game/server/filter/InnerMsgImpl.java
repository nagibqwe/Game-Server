...
public class InnerMsgImpl extends CommandProcessor {
    ...
    protected void doCommand(ICommand command) {
        try {
            boolean isTrue = RobotServer.getInstance().getServerThread().addCommand(command);//压入线程去处理， 保存线程操作
            //如果没有成功， 则进行自处理
            if (!isTrue) {
                command.action();
            }
    ...
    protected void doCommandStatistic(long useTime, ICommand command) {
        if (useTime > 300) {
            if (command instanceof RMessage) {
                ...
                log.error("Обработка сообщения: " + handler.getId() + " — превышение времени: " + useTime);
            } else {
