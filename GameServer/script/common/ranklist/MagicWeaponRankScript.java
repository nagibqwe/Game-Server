...
public class MagicWeaponRankScript implements IRankScript {

    @Override
    public int getId() {
        return ScriptEnum.MagicWeaponRankScript;
    }
    ...
    public int getRankType() {
        return RankType.MAGIC_WEAPON_RANK;
    }
    ...
    public List<RankListMessage.RankInfo.Builder> getRankInfo() {
        ...
        for (Map.Entry<Integer, Long> entry : rankMap.entrySet()) {
            ...
            Manager.rankListManager.deal().buildRankInfo(entry.getKey(), rankPlayer,
                    Manager.playerManager.isOnline(rankPlayer.getRoleId()), String.valueOf(rankPlayer.getMagicWeaponDamage()), rankInfoList,rankPlayer.getMagicWeaponDamage());
        }
        return rankInfoList;
    }
