...
public class HorseRankScript implements IRankScript {

    @Override
    public int getId() {
        return ScriptEnum.HorseRankScript;
    }
    ...
    public List<RankListMessage.RankInfo.Builder> getRankInfo() {
        ...
        for (Map.Entry<Integer, Long> entry : rankMap.entrySet()) {
            ...
            Manager.rankListManager.deal().buildRankInfo(entry.getKey(), rankPlayer,
                    Manager.playerManager.isOnline(rankPlayer.getRoleId()), String.valueOf(rankPlayer.getHorseFightPoint()), rankInfoList,rankPlayer.getHorseFightPoint());
        }
        return rankInfoList;
    }
    ...
    public boolean canRank(RankPlayer rankPlayer) {
        return rankPlayer.getHorseFightPoint() > 0;
    }
