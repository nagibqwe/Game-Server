...
public class ConsumeGoldRankScript implements IRankScript {


    @Override
    public int getId() {
        return ScriptEnum.ConsumeGodRankScript;
    }
    ...
    public int getRankType() {
        return RankType.COMSUME_GOLD_RANK;
    }
    ...
    public List<RankListMessage.RankInfo.Builder> getRankInfo() {
        ...
        for (Map.Entry<Integer, Long> entry : rankMap.entrySet()) {
            ...
            Manager.rankListManager.deal().buildRankInfo(entry.getKey(), rankPlayer,
                    Manager.playerManager.isOnline(rankPlayer.getRoleId()), String.valueOf(rankPlayer.getConsumeGold()), rankInfoList, rankPlayer.getConsumeGold());
        }
        return rankInfoList;
    }
