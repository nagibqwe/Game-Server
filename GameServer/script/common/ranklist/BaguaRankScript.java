...
public class BaguaRankScript implements IRankScript {


    @Override
    public int getId() {
        return ScriptEnum.BaguaRankScript;
    }
    ...
    public List<RankListMessage.RankInfo.Builder> getRankInfo() {
        ...
        for (Map.Entry<Integer, Long> entry : rankMap.entrySet()) {
            ...
            Manager.rankListManager.deal().buildRankInfo(entry.getKey(), rankPlayer,
                    Manager.playerManager.isOnline(rankPlayer.getRoleId()), String.valueOf(rankPlayer.getBaguaPower()), rankInfoList, rankPlayer.getBaguaPower());
        }
        return rankInfoList;
    }
    ...
    public long getCompareValue(RankPlayer rankPlayer) {
        return rankPlayer.getBaguaPower();
    }
}
