...
public class EquipStrengthenScript implements IRankScript {

    @Override
    public int getId() {
        return ScriptEnum.EquipStrengthenRankScript;
    }
    ...
    public int getRankType() {
        return RankType.EQUIPSTRENGTHEN_RANK;
    }
    ...
    public List<RankListMessage.RankInfo.Builder> getRankInfo() {
        ...
        for (Map.Entry<Integer, Long> entry : rankMap.entrySet()) {
            ...
            Manager.rankListManager.deal().buildRankInfo(entry.getKey(), rankPlayer,
                    Manager.playerManager.isOnline(rankPlayer.getRoleId()), String.valueOf(rankPlayer.getEquipStrengthenLv()), rankInfoList,-1);
        }
        return rankInfoList;
    }
