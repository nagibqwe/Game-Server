...
public class ArenaRankScript implements IRankScript {

    @Override
    public int getId() {
        return ScriptEnum.ArenaRankScript;
    }
    ...
    public List<RankListMessage.RankInfo.Builder> getRankInfo() {
        ...
        for (Map.Entry<Integer, Long> entry : rankMap.entrySet()) {
            ...
            Manager.rankListManager.deal().buildRankInfo(entry.getKey(), rankPlayer,
                    Manager.playerManager.isOnline(rankPlayer.getRoleId()), String.valueOf(rankPlayer.getArenaRank()), rankInfoList,-1);
        }
        return rankInfoList;
    }
