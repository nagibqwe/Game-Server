...
public class OfflineEfficiencyRankScript implements IRankScript {

    @Override
    public int getId() {
        return ScriptEnum.OfflineEfficiencyRankScript;
    }
    ...
    public int getRankType() {
        return RankType.OFFLINE_EFFICIENCY_RANK;
    }
    ...
    public List<RankListMessage.RankInfo.Builder> getRankInfo() {
        ...
        for (Map.Entry<Integer, Long> entry : rankMap.entrySet()) {
            ...
            Manager.rankListManager.deal().buildRankInfo(entry.getKey(), rankPlayer,
                    Manager.playerManager.isOnline(rankPlayer.getRoleId()), String.valueOf(rankPlayer.getOfflineEfficiency()), rankInfoList,-1);
        }
        return rankInfoList;
    }
