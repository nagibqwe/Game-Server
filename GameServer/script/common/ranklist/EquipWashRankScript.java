...
public class EquipWashRankScript implements IRankScript {

    @Override
    public int getId() {
        return ScriptEnum.EquipWashRankScript;
    }
    ...
    public int getRankType() {
        return RankType.EQUIPWASH_RANK;
    }
    ...
    public List<RankListMessage.RankInfo.Builder> getRankInfo() {
        ...
        for (Map.Entry<Integer, Long> entry : rankMap.entrySet()) {
            ...
            Manager.rankListManager.deal().buildRankInfo(entry.getKey(), rankPlayer,
                    Manager.playerManager.isOnline(rankPlayer.getRoleId()), String.valueOf(rankPlayer.getEquipWashPer()), rankInfoList, rankPlayer.getEquipWashPer());
        }
        return rankInfoList;
    }
