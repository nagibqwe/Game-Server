...
public class HorseSoulLvRankScript implements IRankScript {

    @Override
    public int getId() {
        return ScriptEnum.HorseSoulLvRankScript;
    }
    ...
    public int getRankType() {
        return RankType.HORSE_SOUL_LV_RANK;
    }
    ...
    public List<RankListMessage.RankInfo.Builder> getRankInfo() {
        ...
        for (Map.Entry<Integer, Long> entry : rankMap.entrySet()) {
            ...
            Manager.rankListManager.deal().buildRankInfo(entry.getKey(), rankPlayer,
                    Manager.playerManager.isOnline(rankPlayer.getRoleId()), String.valueOf(rankPlayer.getHorseSoulLv()), rankInfoList, rankPlayer.getHorseSoulLv());
        }
        return rankInfoList;
    }
