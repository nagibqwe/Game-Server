...
/**
 * 仙甲排行
 * Created by cxl on 2020/10/30.
 */
public class ImmEquipRankScript implements IRankScript {

    @Override
    public int getId() {
        return ScriptEnum.ImmEquipRankScript;
    }
    ...
    public int getRankType() {
        return  RankType.IMEQUIPSTAR_RANK;
    }
    ...
    public List<RankListMessage.RankInfo.Builder> getRankInfo() {
        ...
        for (Map.Entry<Integer, Long> entry : rankMap.entrySet()) {
            ...
            Manager.rankListManager.deal().buildRankInfo(entry.getKey(), rankPlayer,
                    Manager.playerManager.isOnline(rankPlayer.getRoleId()), String.valueOf(rankPlayer.getImmEquipFightPower()), rankInfoList,rankPlayer.getImmEquipFightPower());
        }
        return rankInfoList;
    }
    ...
    public long getCompareValue(RankPlayer rankPlayer) {
        return rankPlayer.getImmEquipFightPower();
    }
}
