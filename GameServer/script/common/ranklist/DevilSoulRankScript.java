...
public class DevilSoulRankScript implements IRankScript {


    @Override
    public int getId() {
        return ScriptEnum.DevilSoulRankScript;
    }
    ...
    public int getRankType() {
        return RankType.DEVIL_SOUL_RANK;
    }
    ...
    public List<RankListMessage.RankInfo.Builder> getRankInfo() {
        ...
        for (Map.Entry<Integer, Long> entry : rankMap.entrySet()) {
            ...
            Manager.rankListManager.deal().buildRankInfo(entry.getKey(), rankPlayer,
                    Manager.playerManager.isOnline(rankPlayer.getRoleId()), String.valueOf(rankPlayer.getDevilSoulPower()), rankInfoList, rankPlayer.getDevilSoulPower());
        }
        return rankInfoList;
    }
