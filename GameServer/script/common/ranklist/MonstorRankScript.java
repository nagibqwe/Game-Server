 ...
 * 神兽战力排行
 * */
public class MonstorRankScript implements IRankScript {

    @Override
    public int getId() {
        return ScriptEnum.MonstorRankScript;
    }
    ...
    public List<RankListMessage.RankInfo.Builder> getRankInfo() {
        ...
        for (Map.Entry<Integer, Long> entry : rankMap.entrySet()) {
            ...
            Manager.rankListManager.deal().buildRankInfo(entry.getKey(), rankPlayer,
                    Manager.playerManager.isOnline(rankPlayer.getRoleId()), String.valueOf(rankPlayer.getMonsterFightPower()), rankInfoList,rankPlayer.getMonsterFightPower());
        }
        return rankInfoList;
    }
