...
public class EquipAllStarRankScript implements IRankScript {

    @Override
    public int getId() {
        return ScriptEnum.EquipAllStarRankScript;
    }
    ...
    public int getRankType() {
        return RankType.EQUIPALLSTAR_RANK;
    }
    ...
    public List<RankListMessage.RankInfo.Builder> getRankInfo() {
        ...
        for (Map.Entry<Integer, Long> entry : rankMap.entrySet()) {
            ...
            Manager.rankListManager.deal().buildRankInfo(entry.getKey(), rankPlayer,
                    Manager.playerManager.isOnline(rankPlayer.getRoleId()), String.valueOf(rankPlayer.getEquipAllStar()), rankInfoList,-1);
        }
        return rankInfoList;
    }
    ...
    public long getCompareValue(RankPlayer rankPlayer) {
        return rankPlayer.getEquipAllStar();
    }
}
