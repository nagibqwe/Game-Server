...
public class HorseEquipRankScript implements IRankScript {


    @Override
    public int getId() {
        return ScriptEnum.HorseEquipRankScript;
    }
    ...
    public int getRankType() {
        return RankType.HORSE_EQUIP_RANK;
    }
    ...
    public List<RankListMessage.RankInfo.Builder> getRankInfo() {
        ...
        for (Map.Entry<Integer, Long> entry : rankMap.entrySet()) {
            ...
            Manager.rankListManager.deal().buildRankInfo(entry.getKey(), rankPlayer,
                    Manager.playerManager.isOnline(rankPlayer.getRoleId()), String.valueOf(rankPlayer.getHorseEquipPower()), rankInfoList, rankPlayer.getHorseEquipPower());
        }
        return rankInfoList;
    }
