...
public class ShiHaiRankScript implements IRankScript {

    @Override
    public int getId() {
        return ScriptEnum.ShiHaiRankScript;
    }
    ...
    public List<RankListMessage.RankInfo.Builder> getRankInfo() {
        ...
        for (Map.Entry<Integer, Long> entry : rankMap.entrySet()) {
            ...
            Manager.rankListManager.deal().buildRankInfo(entry.getKey(), rankPlayer,
                    Manager.playerManager.isOnline(rankPlayer.getRoleId()), String.valueOf(rankPlayer.getShihai()), rankInfoList,-1);
        }
        return rankInfoList;
    }
