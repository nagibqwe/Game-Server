...
/**
 * 宠物战力排行
 * Created by cxl on 2020/10/29.
 */
public class PetRankScript implements IRankScript {


    @Override
    public int getId() {
            return ScriptEnum.PetRankScript;
    }
    ...
    public List<RankListMessage.RankInfo.Builder> getRankInfo() {
        ...
        for (Map.Entry<Integer, Long> entry : rankMap.entrySet()) {
            ...
            Manager.rankListManager.deal().buildRankInfo(entry.getKey(), rankPlayer,
                    Manager.playerManager.isOnline(rankPlayer.getRoleId()), String.valueOf(rankPlayer.getPetFightPower()), rankInfoList,rankPlayer.getPetFightPower());
        }
        return rankInfoList;
    }
