...
public class PetLvRankScript implements IRankScript {


    @Override
    public int getId() {
        return ScriptEnum.PetLvRankScript;
    }
    ...
    public List<RankListMessage.RankInfo.Builder> getRankInfo() {
        ...
        for (Map.Entry<Integer, Long> entry : rankMap.entrySet()) {
            ...
            Manager.rankListManager.deal().buildRankInfo(entry.getKey(), rankPlayer,
                    Manager.playerManager.isOnline(rankPlayer.getRoleId()), String.valueOf(rankPlayer.getPetLv()), rankInfoList, rankPlayer.getPetFightPower());
        }
        return rankInfoList;
    }
