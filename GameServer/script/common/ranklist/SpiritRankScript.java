 ...
 */
public class SpiritRankScript implements IRankScript {
    @Override
    public int getId() {
            return ScriptEnum.SpiritRankScript;
    }
    ...
    public int getRankType() {
        return  RankType.SPIRITSTAR_RANK;
    }
    ...
    public List<RankListMessage.RankInfo.Builder> getRankInfo() {
        ...
        for (Map.Entry<Integer, Long> entry : rankMap.entrySet()) {
            ...
            Manager.rankListManager.deal().buildRankInfo(entry.getKey(), rankPlayer,
                    Manager.playerManager.isOnline(rankPlayer.getRoleId()), String.valueOf(rankPlayer.getSpiritFightPower()), rankInfoList,rankPlayer.getSpiritFightPower());
        }
        return rankInfoList;
    }
