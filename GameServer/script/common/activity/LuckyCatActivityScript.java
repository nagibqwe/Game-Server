 ...
 * 招财猫300010
 */
public class LuckyCatActivityScript implements IActivityScript {

    private static final Logger log = LogManager.getLogger(LuckyCatActivityScript.class);
    ...
    public boolean parseCustomConfig(ActivityConfig actCfg, String customStr) {
        LuckyCatActivity data = JsonUtils.toJavaObject(customStr, LuckyCatActivity.class);
    ...
    public String getActivityDataStr(ActivityConfig actCfg, long roleId) {
        ...
        LuckyCatActivity luckyCatActivity = (LuckyCatActivity) actCfg.getCustomCfgMap().get(configData);
...
//        long rechargeCount = (Long)roleActData.getOrDefault(roleRechargeCount, 0L);
        ...
        Weight weight = randomWeight(weightList);
...
//        int rate = rateList[RandomUtils.random(ranMin,ranMax)];
        ...
        Manager.currencyManager.manager().onDecItemCoin(player, cost, ItemChangeReason.ActivityLuckyCatDec, actionId, ItemCoinType.GemCoin);

        //发放奖励
        Manager.currencyManager.manager().onAddItemCoin(player, ItemCoinType.GemCoin, money, ItemChangeReason.ActivityLuckyCatGet, actionId);
...
//        Manager.biManager.getScript().biActivity(player, ItemChangeReason.ActivityLuckyCatGet, actType, actCfg.getId());
        Manager.biManager.getScript().biActivity(player, BIActiityTypeEnum.LuckyCat, ItemChangeReason.ActivityLuckyCatGet);
    }
    ...
    public void rechargeDeal(Player player,int getGoodsCfgId, int rechargeNum, ActivityConfig actCfg) {
        ...
        ConcurrentHashMap<String, Object> roleActData = Manager.activityManager.deal().getRoleActivityData(player.getId(), actType);
        long rechargeCount = (Long)roleActData.getOrDefault(roleRechargeCount, Manager.activityManager.getRoleTimeRecharge(player.getId(),actCfg));
        int totalCanNum = (Integer)roleActData.getOrDefault(totalCanDrawNum, 0);
        int num = (Integer)roleActData.getOrDefault(drawNum, 0);
...
//        int actType = actCfg.getType();
//
//        for (long roleId:Manager.activityManager.deal().getRoleIdList(actType)) {
//            Player player = Manager.playerManager.getPlayer(roleId);
//            if(player == null){
//                continue;
//            }
//
//            ConcurrentHashMap<String, Object> roleActData = Manager.activityManager.deal().getRoleActivityData(roleId, actType);
//            if(roleActData.get("num")==null){
//                continue;
//            }
//            int num = (int)roleActData.get("num");
//            if(num<=0){
//                continue;
//            }
//
//            //补发对应次数的奖励
//            if(roleActData.get("totalUseNum") == null){
//                roleActData.put("totalUseNum", 0);
//            }
//
//            int totalUseNum = (int)roleActData.get("totalUseNum");
//
//            Integer[] rateList = (Integer[])actCfg.getCustomCfgMap().get("rate");
//            if(rateList.length!=8){
//                continue;
//            }
//
//            List<Integer[]> gearList = (List<Integer[]>)actCfg.getCustomCfgMap().get("gear");
//
//            //执行抽奖逻辑
//            int cost = 0;
//            int ranMin = 0;
...
//
//            //扣除剩余次数
//            roleActData.put("num", 0);
//            roleActData.put("totalUseNum", totalUseNum+num);
//
//            //扣除货币
//            long actionId = IDConfigUtil.getLogId();
//            Manager.currencyManager.manager().onDecItemCoin(player, cost, ItemChangeReason.ActivityLuckyCat, actionId, ItemCoinType.GoldType);

            //发放奖励
//            Manager.currencyManager.manager().onAddItemCoin(player, ItemCoinType.GoldType, money, ItemChangeReason.ActivityLuckyCat, actionId);
//        }
    }
     ...
     * @return
     */
    private Weight randomWeight(List<Weight> params) {
        TreeMap<Float, Weight> weightMap = new TreeMap<>();
        for (int i = 0; i < params.size(); i++) {
            Weight param = params.get(i);
        ...
        SortedMap<Float, Weight> sort = weightMap.tailMap(randomWeight, false);
    ...
    @Override
    public int getId() {
        return ScriptEnum.LuckyCatActivityScript;
    }
    ...
    public void reload() {
        ...
        List<ActivityConfig> actCfgList = Manager.activityManager.deal().getActCfgListByActLogicID(ActivityType.LuckyCat);
        for (ActivityConfig activityConfig:actCfgList) {
            ...
            LuckyCatActivityScript newData = JsonUtils.toJavaObject(customStr, LuckyCatActivityScript.class);
