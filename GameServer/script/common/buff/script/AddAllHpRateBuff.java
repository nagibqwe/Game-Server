...
public class AddAllHpRateBuff implements IScript, IBuffBehavior {
    ...
    public int action(Buff buff, Fighter attacker, Fighter owner) {
        ...
        long add = (long) (owner.getAttribute().MaxHP() * (config.getParam1() / 10000f));
        MapUtils.sendBuffHp(attacker, owner, (int) add);

        long need = owner.getAttribute().MaxHP() - owner.getCurHp();

        long min = Math.min(add, need);
    ...
    @Override
    public int getId() {
        return ScriptEnum.AddAllHpRateBuff;
    }
