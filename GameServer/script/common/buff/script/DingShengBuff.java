 ...
 */
public class DingShengBuff implements IScript, IBuffBehavior {
    ...
    public int add(Buff buff,Fighter source, Fighter target) {
        ...
        if (!target.isDie()) {
            ...
            if (BehaviorManager.HasBehavior((Entity) target, BehaviorType.Revive)) {
                log.error(source.getName() + "(" + source.getId() + ") 对 " + target.getName() + "(" + target.getId() + ")施加定身技能id=" + buff.getBuffId() + "时， 正在死亡复活状态！");
            } else {
                BehaviorManager.CancelAllBehavior((Entity) target);
            }
        }
//        log.info(source.getName() + "(" + source.getId() + ") 对 " + target.getName() + "(" + target.getId() + ")施加定身技能id=" + getBuffId());
        //清除当前的引导技能
