...
import com.data.bean.Cfg_NatureMagic_Bean;
...
public class MagicAttributeScript implements IAttributeScript {

    @Override
    public int getId() {
        return ScriptEnum.MagicAttributeScript;
    }
    ...
    public PlayerAttributeType getType() {
        return PlayerAttributeType.Magic;
    }
    ...
    public BaseIntAttribute getPlayerAttribute(Player player, boolean sycRank) {
        ...
        if (!Manager.controlManager.deal().isOpenFunction(player, FunctionStart.NatureMagic)) {
            return att;
        }
        ...
        if (sycRank) {
            ...
            player.getMagic().setPower(power);
            Manager.rankListManager.deal().setMagicPower(player, power);
        }
        return att;
    }

    private void calculateMagicAttribute(Player player, BaseIntAttribute att) {
        Nature magic = player.getMagic();
        Cfg_NatureMagic_Bean magicBean = CfgManager.getCfg_NatureMagic_Container().getValueByKey(magic.getCurrentId());
        if (magicBean == null) {
            return;
        }
        for (ReadArray array : magicBean.getAttribute().getValuees()) {
            if (array.size() >= 3) {
        ...
        for (int i = 1; i < systemAtt.getLength(); i++) {
            ...
            switch (i) {
                case ATTR_Magic_Hit:
                    AttributeUtils.attributeEnlarge(att, ATTR_Hit, rate);
                    break;
                case ATTR_Magic_HitBreak:
                    AttributeUtils.attributeEnlarge(att, ATTR_HitBreak, rate);
                    break;
                case ATTR_Magic_Critical:
                    AttributeUtils.attributeEnlarge(att, ATTR_CritRate, rate);
                    break;
                case ATTR_Magic_Resilience:
                    AttributeUtils.attributeEnlarge(att, ATTR_Resilience, rate);
                    break;
                default:
                    break;
            }
        ...
        for (Drug drug : player.getMagic().getDrugs().values()) {
        ...
        /*
          ...
          */
        for (Huaxin huaxin : player.getMagic().getHuaxins().values()) {
            int level = huaxin.getLevel() + 1;
            Cfg_HuaxingMagic_Bean huaxingMagicBean = CfgManager.getCfg_HuaxingMagic_Container().getValueByKey(huaxin.getExcelId());
            ReadIntegerArrayEs arrays = huaxingMagicBean.getRent_att();
            if (null != arrays) {
