 ...
 * @author zenghai
 */
public class RunBehaviorScript implements IScript, IBehavior {

    protected Logger log = LogManager.getLogger("BaseBehavior");

    @Override
    public int getId() {
        return ScriptEnum.RunBehaviorCommonScript;
    }
    ...
    public void action(BaseBehavior behavior) {
        ...
        while (!mover.getRoads().isEmpty()) {
            ...
            if (moveDis > dis) {
                ...
                if (Utils.isCanMove(map,mover.gainCurPos(), dirPos)) {
                ...
                mover.clearRoads();
                //LOGGER.error("[" + mover.getName() + "]方向移动行为执行old=" + old + "new=" + dirPos + "moveDis=" + moveDis + "speed=" + speed + "time=" + behavior.getOffset());
                return;
            }
        }
