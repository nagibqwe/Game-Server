	...
	$.ajax({  
        ...
        success: function(response) { 
        	if (response.length > 0){  
                var menu_section =  '<div class="menu_section">' +
					                '<h3>Third Party</h3>' +
        		...
        		for (var i = 0; i < response.length; ++i) {
        			var li = ''
        			var vlis = ''	
        			var nav_third_number = response[i]['nav_third_number']	
        			if (nav_third_number.length){
        				var ul = ''
        				var vli = ''
            			for (var x = 0; x < nav_third_number.length; ++x) {
            				 vli = '<li><a href="/nav/third/'+ nav_third_number[x]["id"] +'/">' + nav_third_number[x]["nav_name"] + '</a></li>'
            				 vlis = vlis + vli
            			}         				
        				ul += '<ul class="nav child_menu">' + vlis + '</ul>'
        				li += '<li><a><i class="'+ response[i]['icon'] +'"></i> '+response[i]['type_name']+' <span class="fa fa-chevron-down"></span></a>'+ul +'</li>'
        			}
        			lis = lis + li
        		}                 
				menu_section = menu_section + lis	+ '</ul> </div>'	
				$("#thirdParty").html(menu_section)
				$("#thirdParty").show()
        	}
...
function init_chart_doughnut(ids,labels,data) {
...
function init_daterangepicker_single_call() {
    ...
    function(a, b, c) {
    }), $("input.has-feedback-left").daterangepicker({
        singleDatePicker: !0,
...
function init_daterangepicker_reservation() {
    "undefined" != typeof $.fn.daterangepicker &&  ($("#reservation").daterangepicker(null,
    function(a, b, c) {
        console.log(a.toISOString(), b.toISOString(), c)
...
function init_validator() {
    "undefined" != typeof validator && (validator.message.date = "not a real date", $("form").on("blur", "input[required], input.optional, select.required", validator.checkField).on("change", "select.required", validator.checkField).on("keypress", "input[required][pattern]", validator.keypress), $(".multi.required").on("keyup blur", "input",
    function() {
