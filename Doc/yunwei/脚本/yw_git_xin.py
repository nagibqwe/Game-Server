...
def goujian(dirname):
    ...
    r = svn.remote.RemoteClient("http://10.0.1.151/svn/MMORPG/TianZhiJin/branches/1.2.0.0_0312_2019.03.10_Main/Server",username="luowei",password="yZ1r67T7")
    r.checkout(dirname)
    #os.chdir(dirname)
    #cmd = "source /etc/profile && /usr/local/ant/bin/ant -f build-ant.xml | tee build.log"
    #dirs = ["GameCfg","GameCore","Message","GameServer"]
    #for d in dirs:
