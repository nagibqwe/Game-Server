...
(function ($) {
    $.extend({
    	...
    	table: {
            ...
            responseHandler: function(res) {
                ...
                if (res.code == web_status.SUCCESS) {
                    if ($.common.isNotEmpty(table.options.sidePagination) && table.options.sidePagination == 'client') {
                        ...
                        return { rows: res.rows, total: res.total, paramMap:res.paramMap};
                    }
                }else if(res.code == -1){
                    $.modal.msgError(res.msg);
                    return { rows: [], total: 0, paramMap:{}};
                } else {
            ...
            // 导出数据(默认是导出服务器所有数据)(tableId和isServer是后面新加参数)
            exportExcel: function(formId,tableId,isServer) {
                table.set(tableId);
                var arrays = $.table.selectColumns("id");
                if (arrays.length > 0){
                    $.modal.confirm("确定导出" + arrays.length + "条数据吗？", function () {
                        $.exportData.export(formId,tableId,isServer,arrays);
                    });
                }else {
    	...
    	modal: {
            ...
            // 弹出层指定宽度
            open: function (title, url, width, height, callback,successCallback) {
            	// 如果是移动端，就使用自适应大小弹窗
            	if ($.common.isMobile()) {
            	    width = 'auto';
            	    height = 'auto';
