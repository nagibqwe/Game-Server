...
<head>
    ...
    <title>${msg['activity.login.title']}</title>
    ...
    $(function() {      	    	
    	...
    	$("#deleReward").click(function() {
    		var tempItem = $("#rewardList").val();
    	...
    	$("#addSubmit").click(function() {
    		...
    		if ($('#activity_add').validationEngine('validate')) {
    			...
    			var sbTime = new Date($("#mailStartTime").val().replace(/\-/g, "\/"));
    			var seTime = new Date($("#mailEndTime").val().replace(/\-/g, "\/"));
    			...
    			} else if (seTime <= sbTime) {
    				alert("${msg['activity.login.loginTimeFailure']}");
    			//} else if (seTime > curTime) {
    				//alert("${msg['activity.login.loginTimeGtNow']}");
    			} else {
    				$("#isTemplate").val("0");
    				var itemPrice = 0;
    	    		var rewardList = $("#rewardList").val();
    	    		if(rewardList != ""){
    				...
    				$.post(base + "/activity/addLoginMail", $("#activity_add").serialize(), function(data, status) {
    					alert(data.msg);
    					if (data.ok) {
    	...
    	$("#isServer").change(function() {
        	var isNotice = $("#isServer").val();      	
        	if (isNotice == 1) {
        		$("#sidinput").show();
        	} else {
        		$("#sidinput").hide();
...
<body>
	...
	<table width="95%" height="95%" align="center">
		<tr><td valign="top"><br/>
		    ...
		    <strong>——>${msg['activity.login.title']}</strong><br/><br/>
        </td></tr>
        ...
        <tr><td>       
            ...
            <table frame="box" width="95%" cellspacing="10">
                <tr align="left">
                <td colspan="3"><strong>${msg['activity.login.baseMess']}</strong></td>
                ...
                <tr align="left">
                <td colspan="4">${msg['activity.login.mailTitle']}&nbsp;&nbsp;&nbsp;&nbsp;<input id="name" name="name" type="text" size="92" class="validate[required]"/></td>
                </tr>
                                
                <tr align="left">             
                <td colspan="4"><label style="vertical-align:top" >${msg['activity.login.mailcontent']}</label>&nbsp;&nbsp;&nbsp;
                    <textarea rows="5" cols="80" id="panelText" name="panelText" class="validate[required]"></textarea>
                </td>
                </tr> 
                
                <tr align="left">
                <td colspan="4">${msg['activity.login.fujianItem']}&nbsp;&nbsp;&nbsp;&nbsp;<input name="rewardList" id="rewardList" type="text" size="92" readonly/>
                    <input type="button" id="addItem" value="+"/><input type="button" id="deleReward" value="-"/><input type="button" id="empty" value="${msg['activity.empty']}"/>
                    <!-- <font color="red">${msg['activity.login.fujianLimit']}</font> -->
                </td>
                </tr>
                                
                <tr align="left">
                <td colspan="1">${msg['activity.login.sendStarttime']}<input name="beginTime" id="beginTime" type="text" class="datetimepicker validate[required]" readonly/></td>
                <td colspan="1">${msg['activity.login.sendEndtime']}<input name="endTime" id="endTime" type="text" class="datetimepicker validate[required]" readonly/>
                    <font color="red">${msg['activity.login.sendTimeInfo']}</font>
                </td>
                </tr>
                
                <tr align="left">
                <td colspan="4"><strong>${msg['activity.login.conditionMess']}</strong></td>
                </tr>  
                
                <tr align="left">                
                <td colspan="1">${msg['activity.login.isserver']}<select id="isServer" name="isServer" class="validate[required]"><option value="0" selected="selected">${msg['activity.login.noserver']}</option><option value="1">${msg['activity.login.yesserver']}</option></select></td>
                <td colspan="3" id="sidinput" style="display:none">${msg['activity.login.sids']}<input id="sids" name="sids" type="text" class="validate[required]"/>&nbsp;&nbsp;<font color="red">${msg['activity.login.sidtip']}</font></td>  
                </tr>           
                
                <tr align="left">                
                <td colspan="1">${msg['activity.login.timeStart']}<input name="mailStartTime" id="mailStartTime" type="text" class="datetimepicker validate[required]" readonly/></td>
                <td colspan="1">${msg['activity.login.timeEnd']}<input name="mailEndTime" id="mailEndTime" type="text" class="datetimepicker validate[required]" readonly/></td>     
                </tr>
                
                <tr align="left">
                <td colspan="2">
                    <%-- ${msg['activity.login.jimin']}<input name="mailminji" id="mailminji" type="text" class="validate[required,custom[integer],min[0],max[10]]" value="0" size="5"/> --%>
                    &nbsp;${msg['activity.login.levelMin']}<input name="mailminlevel" id="mailminlevel" type="text" class="validate[required,custom[integer],min[1]]" value="1" size="5"/>
                    <%-- &nbsp;${msg['activity.login.jimax']}<input name="mailmaxji" id="mailmaxji" type="text" class="validate[required,custom[integer],min[0],max[10]]" value="10" size="5"/> --%>
                    &nbsp;${msg['activity.login.levelMax']}<input name="mailmaxlevel" id="mailmaxlevel" type="text" class="validate[required,custom[integer],min[1],max[800]]" value="800" size="5"/>
                </td>    
                </tr>              
