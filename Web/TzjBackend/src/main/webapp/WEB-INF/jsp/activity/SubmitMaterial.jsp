<%@page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>

<head>
    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    <title>${msg['activity.submitMaterial']}</title>

    <link rel="stylesheet" href="${base}/css/activity.css" type="text/css" />
    <link rel="stylesheet" href="${base}/css/common.css" type="text/css" />
    <link rel="stylesheet" href="${base}/css/boxy.css" type="text/css" />
    <link rel="stylesheet" href="${base}/css/validationEngine.jquery.css">
    <link rel="stylesheet" href="${base}/css/bootstrap/css/bootstrap.css">
    <link rel="stylesheet" href="${base}/css/bootstrap/css/bootstrap-datetimepicker.min.css">

    <script type="text/javascript" src="${base}/js/jquery/jquery-2.1.4.min.js"></script>

    <script type="text/javascript" src="${base}/js/jquery/jquery.boxy.js"></script>
    <script type="text/javascript" src="${base}/js/jquery/jquery.validationEngine-zh_CN.js"></script>
    <script type="text/javascript" src="${base}/js/jquery/jquery.validationEngine.js"></script>
    <script type="text/javascript" src="${base}/css/bootstrap/js/bootstrap.min.js"></script>
    <script type="text/javascript" src="${base}/css/bootstrap/js/datetimepicker/bootstrap-datetimepicker.min.js"></script>
    <script type="text/javascript" src="${base}/css/bootstrap/js/datetimepicker/language/bootstrap-datetimepicker.zh-CN.js"></script>
    <jsp:include page="operator.jsp"/>
    <script type="text/javascript"> 
    var base = '${base}';
    var actType = 102;
    var itemMap = new Object();//用来存放itemId和对应的蓝钻价值
    
    var allConditionIndex = 1;
    var perConditionIndex = 1 ;
    function add_condition(index,type){
    	var condition = new Array();
    	if(index > 10) {
    		alert("${msg['activity.di.tenLimit']}");
    		return;
    	}
    	var condition_html = "";
    	if(type == "allServer"){
    		condition_html = "<div id=\"alConditionList"+index+"\">${msg['activity.di.first']}"+index+"${msg['activity.di.type']}\
   		 	&nbsp;&nbsp;${msg['activity.submitMaterial.rewardType']}<select id=\"alRewardType"+index+"\" name=\"alRewardType"+index+"\" onchange=\"getCondition("+index+")\" class=\"validate[required]\">\
   		 	<option value=\"0\">${msg['activity.receive.condition.select']}</option>\
   		 	<option value=\"1\">${msg['activity.submitMaterial.doubleExperise']}</option>\
            <option value=\"2\">${msg['activity.submitMaterial.allServerReward']}</option>\
            <option value=\"3\">${msg['activity.submitMaterial.instanceDoubleDrop']}</option>\
            <option value=\"4\">${msg['activity.submitMaterial.bossReset']}</option></select>\
            <span id=\"alConditionDetail"+index+"\"></span>\
            &nbsp;&nbsp;${msg['activity.submitMaterial.num']}<input id=\"alNum"+index+"\" name=\"alNum"+index+"\" type=\"text\" class=\"validate[required,custom[integer],min[1],max[99999999]]\"/>\
           </div>";
    		$("#selectAllServerCondition").append(condition_html);
        	allConditionIndex += 1;
    	}else if(type == "personal"){
    		condition_html = "<div id=\"perConditionList"+index+"\">${msg['activity.di.first']}"+index+"${msg['activity.di.type']}\
    		<input name=\"rewardList"+index+"\" id=\"rewardList"+index+"\" type=\"text\" size=\"60\" class=\"validate[required]\" readonly/>\
 	        <input type=\"button\" id=\"addItem"+index+"\" value=\"+\" onclick=\"itemShowBox("+index+");\"/><input type=\"button\" value=\"-\" onclick=\"deleItem("+index+");\"/>\
 	        <input type=\"button\" id=\"empty"+index+"\" onclick=\"rewardEmpty("+index+");\" value=\"${msg['activity.empty']}\"/>\
            &nbsp;&nbsp;${msg['activity.submitMaterial.num']}<input id=\"perNum"+index+"\" name=\"perNum"+index+"\" type=\"text\" class=\"validate[required,custom[integer],min[1],max[99999999]]\"/>\
           </div>";
        	$("#selectPersonalCondition").append(condition_html);
        	perConditionIndex += 1;
        }
    	
    }
    
    function delete_condition(index,type){
    	if(index >= 1){
    		if(type == "allServer"){
    			$("#alConditionList"+index).remove();
    			allConditionIndex -= 1;
    		}else if(type == "personal"){
    			$("#perConditionList"+index).remove();
    			perConditionIndex -= 1;
    		}
    		
    	}
    }
    
    function getCondition(index) {  
    	//console.log("con==="+con);
   		var condition = $("#alRewardType"+index).val();    		
   		var detail_condition = ""; 
   		//console.log("condition==="+condition);
   		switch(condition) {
   		case "2":
   			detail_condition ="${msg['activity.submitMaterial.itemId']}\
   			<input name=\"alItemId"+index+"\" id=\"alItemId"+index+"\" type=\"text\" size=\"60\" class=\"validate[required]\" readonly/>\
            <input type=\"button\" id=\"addAlItemId"+index+"\" value=\"+\" onclick=\"itemsShowBox("+index+");\"/>\
            <input type=\"button\" id=\"delAlItemId"+index+"\" value=\"-\" onclick=\"deleItems("+index+");\" />\
            <input type=\"button\" id=\"alItemIdEmpty"+index+"\" value=\"${msg['activity.empty']}\" onclick=\"itemsEmpty("+index+");\"/>";    			
   			break;
   		}
   		$("#alConditionDetail"+index).html(detail_condition);
    		
    }
    
    function itemsEmpty(index) {
		$("#alItemId"+index).val("");
	}
	
	function deleItems(index) {
		var tempItem = $("#alItemId"+index).val();
		if (tempItem != null && tempItem != "") {
			var strItem = "";
			var idx = tempItem.lastIndexOf(";");
			if (idx > -1) {
				strItem = tempItem.slice(0, idx);
			}
			$("#alItemId"+index).val(strItem);
		}
	}
	
	function itemsShowBox(index) {
		$.ajax({
			url : base + "/global/getItemInfo",
			dataType : "json",
			success : function(jsonData) {
				var data = eval('(' + jsonData + ')');	
				
				var boxy_html = "<form id='add_item' method='post' action='#'><div>${msg['activity.item.select']}<input id='itemId' list='itemId1' class='validate[required]' type='text' name='itemId_list' /><datalist id='itemId1'><option value='${msg['activity.item.itemId']}' selected='selected'>----${msg['activity.item']}----</option>";
				var idList = new Array();
				$.each(data, function(i) {
					var item = data[i];					
					//console.log(item.itemId + "," + item.itemName + "," + item.itemMax);
					boxy_html += "<option value='" + item.itemId + "'>" + item.itemId + "_" + item.itemName + "_" + item.itemPrice + "_" + item.itemMax + "</option>";
					idList.push(item.itemId + "");
				});
				
				boxy_html += "</datalist>*<br/>${msg['activity.item.num']}<input type='text' id='itemNum' class='validate[required,custom[integer],min[1],max[2100000000]]'/>*\
						<br/>${msg['activity.item.bind']}<select id='isBind'><option value='' selected='selected'></option><option value='1'>${msg['activity.yes']}</option><option value='0'>${msg['activity.no']}</option></select>\
						<br/>${msg['activity.item.losttime']}<input id='lostTime' type='text' class='datetimepicker1' readonly/>\
						<br/>${msg['activity.item.strenlv']}<input id='strenLevel' type='text' class='validate[custom[integer],min[0]]'/>\
					    <br/>${msg['activity.item.starlv']}<input id='starLevel' type='text' class='validate[custom[integer],min[0]]'/>\
					    <br/>${msg['activity.item.career']}<select id='career'><option value='' selected='selected'></option><option value='0'>${msg['activity.item.career.a']}</option><option value='1'>${msg['activity.item.career.b']}</option><option value='2'>${msg['activity.item.career.c']}</option><option value='3'>${msg['activity.item.career.d']}</option><option value='4'>${msg['activity.item.career.e']}</option><option value='5'>${msg['activity.item.career.f']}</option></select>\
					    <br/><br/><input id='add' type='button' value='${msg['activity.item.add']}'/></div></form>";
										
		    	var boxy = new Boxy(boxy_html,
		        {
		    		title:"${msg['activity.item.select']}",        //Box标题
		    		closeText:"[${msg['activity.item.cancel']}]",     //Box关闭文字
		    		modal:true,           //Box背景是否变暗，此处必须为true，为false的话Boxy选择框会遮挡住弹出的时间选择控件
		    		fixed:false,          //Box窗口是否固定
		    		cache:false,          //Box是否被遮挡
		    		draggable:true,       //Box是否可以拖动，要定义title才有效，设定了modal就无效
		    		center:true,          //Box是否居中
		    		unloadOnHide:true,    //点击"取消"后box是隐藏并没有卸载掉，此处设置在隐藏后则卸掉
		    		afterShow:function() {
		    			$('.datetimepicker1').datetimepicker({  //日期时间选择器配置
		    				language:  'zh-CN',
		    			    format: 'yyyy-mm-dd hh:ii',    	    
		    			    todayBtn: 1,          
		    			    autoclose:true
		    			});    		    			
		    			$("#add_item").validationEngine();
		    			
		    			$("#add").click(function() {
		    				if ($('#add_item').validationEngine('validate')) { //物品信息验证通过后组装物品信息为字串填到奖励物品栏中
		    					if(jQuery.inArray($("#itemId").val(), idList) == -1){
		    						alert("${msg['activity.item.idnotexist']}");
		    						return ;
		    					}
		    				
		    					var itemInfo = $("#alItemId"+index).val();		    					
		    					if (itemInfo != '') {
		    						itemInfo += ";";
		    					}

		    					itemInfo += $("#itemId").val() + "," + $("#itemNum").val();
		    					
		    					if ($("#career").val() != "") {
		    						itemInfo += ","  + $("#isBind").val() + "," + $("#lostTime").val() + "," + $("#strenLevel").val() + "," + $("#starLevel").val() + "," + $("#career").val();
		    					} else if ($("#starLevel").val() != "") {
		    						itemInfo += ","  + $("#isBind").val() + "," + $("#lostTime").val() + "," + $("#strenLevel").val() + "," + $("#starLevel").val();
		    					} else if ($("#strenLevel").val() != "") {
		    						itemInfo += ","  + $("#isBind").val() + "," + $("#lostTime").val() + "," + $("#strenLevel").val();
		    					} else if ($("#lostTime").val() != "") {
		    						itemInfo += ","  + $("#isBind").val() + "," + $("#lostTime").val();
		    					} else if ($("#isBind").val() != "") {
		    						itemInfo += ","  + $("#isBind").val();
		    					}		    					
		    					
		    					//console.log(itemInfo);
		    					$("#alItemId"+index).val(itemInfo);
		    					
		    					Boxy.get(this).hideAndUnload(); //点击发布后隐藏并删除此对话框，直接unload()的话，模态框的暗背景不会消失
		    				}
		    			});		    			
		    			
		    		} //打开对话框后执行的
		        });		    	
		    	boxy.centerAt(600, 300);
		    	boxy.resize(350, 220);
			}
		});
	}
    
    $(function() {      	    	
    	activity_reload(actType); //js文件里无法直接取得语言配置，只好又搬回到JSP页面里来
    	showPage(1, actType);
    	itemMap = getItemInfoList();
    	activity_template_reload(actType);
    	
    	$("#activity_add").validationEngine();
    	
    	$('.datetimepicker').datetimepicker({  //日期时间选择器配置
    		language:  'zh-CN',
    	    format: 'yyyy-mm-dd hh:ii',    	    
    	    todayBtn: 1,          
    	    autoclose:true
    	}); 
    	
    	$.ajax({
			url : base + "/global/getItemInfo",
			dataType : "json",
			success : function(jsonData) {
				var data = eval('(' + jsonData + ')');	
				var item_option = "";
				$.each(data, function(i) {
					var item = data[i];					
					item_option += "<option value='" + item.itemId + "'>" + item.itemId + "_" + item.itemName + "_" + item.itemMax + "</option>";
				});
				$("#materialIdList").html(item_option);
			}
		});
    	
    	add_condition(allConditionIndex,"allServer");
    	$("#addAllServerCondition").click(function() {    
    		add_condition(allConditionIndex,"allServer");
    	});
    	$("#deleteAllServerCondition").click(function() {
    		delete_condition(allConditionIndex-1,"allServer");	
    	});
    	
    	add_condition(perConditionIndex,"personal");
    	$("#addPersonalCondition").click(function() {    
    		add_condition(perConditionIndex,"personal");
    	});
    	$("#deletePersonalCondition").click(function() {
    		delete_condition(perConditionIndex-1,"personal");	
    	});
    	
    	/* 个人提交奖励*/
    	$("#addActivityItem").click(function() {
    		$.ajax({
    			url : base + "/global/getItemInfo",
    			dataType : "json",
    			success : function(jsonData) {
    				var data = eval('(' + jsonData + ')');	
    				
    				var boxy_html = "<form id='add_item' method='post' action='#'><div>${msg['activity.item.select']}<input id='itemId' list='itemId1' class='validate[required]' type='text' name='itemId_list' /><datalist id='itemId1'><option value='${msg['activity.item.itemId']}' selected='selected'>----${msg['activity.item']}----</option>";
    				var idList = new Array();
    				$.each(data, function(i) {
    					var item = data[i];					
    					//console.log(item.itemId + "," + item.itemName + "," + item.itemMax);
    					boxy_html += "<option value='" + item.itemId + "'>" + item.itemId + "_" + item.itemName + "_" + item.itemPrice + "_" + item.itemMax + "</option>";
    					idList.push(item.itemId + "");
    				});
    				
    				boxy_html += "</datalist>*<br/>${msg['activity.item.num']}<input type='text' id='itemNum' class='validate[required,custom[integer],min[1],max[2100000000]]'/>*\
						<br/>${msg['activity.item.bind']}<select id='isBind'><option value='' selected='selected'></option><option value='1'>${msg['activity.yes']}</option><option value='0'>${msg['activity.no']}</option></select>\
						<br/>${msg['activity.item.losttime']}<input id='lostTime' type='text' class='datetimepicker1' readonly/>\
						<br/>${msg['activity.item.strenlv']}<input id='strenLevel' type='text' class='validate[custom[integer],min[0]]'/>\
					    <br/>${msg['activity.item.starlv']}<input id='starLevel' type='text' class='validate[custom[integer],min[0]]'/>\
					    <br/>${msg['activity.item.career']}<select id='career'><option value='' selected='selected'></option><option value='0'>${msg['activity.item.career.a']}</option><option value='1'>${msg['activity.item.career.b']}</option><option value='2'>${msg['activity.item.career.c']}</option><option value='3'>${msg['activity.item.career.d']}</option><option value='4'>${msg['activity.item.career.e']}</option><option value='5'>${msg['activity.item.career.f']}</option></select>\
					    <br/><br/><input id='add' type='button' value='${msg['activity.item.add']}'/></div></form>";
    				
    		    	var boxy = new Boxy(boxy_html,
    		        {
    		    		title:"${msg['activity.item.select']}",        //Box标题
    		    		closeText:"[${msg['activity.item.cancel']}]",     //Box关闭文字
    		    		modal:true,           //Box背景是否变暗，此处必须为true，为false的话Boxy选择框会遮挡住弹出的时间选择控件
    		    		fixed:false,          //Box窗口是否固定
    		    		cache:false,          //Box是否被遮挡
    		    		draggable:true,       //Box是否可以拖动，要定义title才有效，设定了modal就无效
    		    		center:true,          //Box是否居中
    		    		unloadOnHide:true,    //点击"取消"后box是隐藏并没有卸载掉，此处设置在隐藏后则卸掉
    		    		afterShow:function() {    		    			
    		    			$("#add_item").validationEngine();
    		    			
    		    			$("#add").click(function() {
    		    				if ($('#add_item').validationEngine('validate')) { //物品信息验证通过后组装物品信息为字串填到奖励物品栏中
    		    					if(jQuery.inArray($("#itemId").val(), idList) == -1){
    		    						alert("${msg['activity.item.idnotexist']}");
    		    						return ;
    		    					}
    		    				
    		    					var itemInfo = $("#personalRewardList").val();		    					
    		    					if (itemInfo != '') {
    		    						itemInfo += ";";
    		    					}		    					
    		    					itemInfo += $("#itemId").val() + "," + $("#itemNum").val();
    		    					
    		    					if ($("#career").val() != "") {
    		    						itemInfo += ","  + $("#isBind").val() + "," + $("#lostTime").val() + "," + $("#strenLevel").val() + "," + $("#starLevel").val() + "," + $("#career").val();
    		    					} else if ($("#starLevel").val() != "") {
    		    						itemInfo += ","  + $("#isBind").val() + "," + $("#lostTime").val() + "," + $("#strenLevel").val() + "," + $("#starLevel").val();
    		    					} else if ($("#strenLevel").val() != "") {
    		    						itemInfo += ","  + $("#isBind").val() + "," + $("#lostTime").val() + "," + $("#strenLevel").val();
    		    					} else if ($("#lostTime").val() != "") {
    		    						itemInfo += ","  + $("#isBind").val() + "," + $("#lostTime").val();
    		    					} else if ($("#isBind").val() != "") {
    		    						itemInfo += ","  + $("#isBind").val();
    		    					}		    					
    		    					
    		    					//console.log(itemInfo);
    		    					$("#personalRewardList").val(itemInfo);
    		    					
    		    					Boxy.get(this).hideAndUnload(); //点击发布后隐藏并删除此对话框，直接unload()的话，模态框的暗背景不会消失
    		    				}
    		    			});		    			
    		    			
    		    		} //打开对话框后执行的
    		        });		    	
    		    	boxy.centerAt(900, 300);
    		    	boxy.resize(300, 200);
    			}
    		});
    	});
        $("#deleActivityReward").click(function() {
    		var tempItem = $("#personalRewardList").val();
    		if (tempItem != null && tempItem != "") {
    			var strItem = "";
    			var idx = tempItem.lastIndexOf(";");
    			if (idx > -1) {
    				strItem = tempItem.slice(0, idx);
    			}
    			$("#personalRewardList").val(strItem);
    		}
    	});
    	
    	$("#activityEmpty").click(function() {
    		$("#personalRewardList").val("");
    	});
    	
    	/* 个人提交展示内容 */
    	
    	$("#addItemId").click(function() {
    		$.ajax({
    			url : base + "/global/getItemInfo",
    			dataType : "json",
    			success : function(jsonData) {
    				var data = eval('(' + jsonData + ')');	
    				
    				var boxy_html = "<form id='add_item' method='post' action='#'><div>${msg['activity.item.select']}<input id='itemId' list='itemId1' class='validate[required]' type='text' name='itemId_list' /><datalist id='itemId1'><option value='${msg['activity.item.itemId']}' selected='selected'>----${msg['activity.item']}----</option>";
    				var idList = new Array();
    				$.each(data, function(i) {
    					var item = data[i];					
    					//console.log(item.itemId + "," + item.itemName + "," + item.itemMax);
    					boxy_html += "<option value='" + item.itemId + "'>" + item.itemId + "_" + item.itemName + "_" + item.itemPrice + "_" + item.itemMax + "</option>";
    					idList.push(item.itemId + "");
    				});
    				
    				boxy_html += "</datalist>*<br/><input id='add' type='button' value='${msg['activity.item.add']}'/></div></form>";
    				
    		    	var boxy = new Boxy(boxy_html,
    		        {
    		    		title:"${msg['activity.item.select']}",        //Box标题
    		    		closeText:"[${msg['activity.item.cancel']}]",     //Box关闭文字
    		    		modal:true,           //Box背景是否变暗，此处必须为true，为false的话Boxy选择框会遮挡住弹出的时间选择控件
    		    		fixed:false,          //Box窗口是否固定
    		    		cache:false,          //Box是否被遮挡
    		    		draggable:true,       //Box是否可以拖动，要定义title才有效，设定了modal就无效
    		    		center:true,          //Box是否居中
    		    		unloadOnHide:true,    //点击"取消"后box是隐藏并没有卸载掉，此处设置在隐藏后则卸掉
    		    		afterShow:function() {    		    			
    		    			$("#add_item").validationEngine();
    		    			
    		    			$("#add").click(function() {
    		    				if ($('#add_item').validationEngine('validate')) { //物品信息验证通过后组装物品信息为字串填到奖励物品栏中
    		    					if(jQuery.inArray($("#itemId").val(), idList) == -1){
    		    						alert("${msg['activity.item.idnotexist']}");
    		    						return ;
    		    					}
    		    				
    		    					var itemInfo = $("#itemIds").val();		    					
    		    					if (itemInfo != '') {
    		    						itemInfo += ";";
    		    					}		    					
    		    					itemInfo += $("#itemId").val();	    					
    		    					
    		    					//console.log(itemInfo);
    		    					$("#itemIds").val(itemInfo);
    		    					
    		    					Boxy.get(this).hideAndUnload(); //点击发布后隐藏并删除此对话框，直接unload()的话，模态框的暗背景不会消失
    		    				}
    		    			});		    			
    		    			
    		    		} //打开对话框后执行的
    		        });		    	
    		    	boxy.centerAt(900, 300);
    		    	boxy.resize(300, 200);
    			}
    		});
    	});
        $("#deleItemId").click(function() {
    		var tempItem = $("#itemIds").val();
    		if (tempItem != null && tempItem != "") {
    			var strItem = "";
    			var idx = tempItem.lastIndexOf(";");
    			if (idx > -1) {
    				strItem = tempItem.slice(0, idx);
    			}
    			$("#itemIds").val(strItem);
    		}
    	});
    	
    	$("#itemIdEmpty").click(function() {
    		$("#itemIds").val("");
    	});
    	
    	
    	$("#addSubmit").click(function() {
    		if ($('#activity_add').validationEngine('validate')) {

    			var curTime = new Date();
    			
    			var bTime = new Date($("#beginTime").val().replace(/\-/g, "\/"));
    			var eTime = new Date($("#endTime").val().replace(/\-/g, "\/"));
    			   			
    			if (eTime <= bTime) {
    				alert("${msg['activity.ebtime.alert']}");
    			} else if (eTime <= curTime) {
    				alert("${msg['activity.ectime.alert']}");
    			} else {
    				$("#isTemplate").val("0");
    				var itemPrice = 0;
    				var n = 1;
    	    		while(n){
    	    			var rewardList = $("#rewardList"+n).val();
    	    			//console.log("====="+rewardList);
        	    		if(rewardList != "" && rewardList != undefined){
        	    			var rewardLists = rewardList.split(";");
        	        		for(var i = 0 ; i < rewardLists.length ; i++){
        	        			var itemObj = rewardLists[i].split(",");
        	        			itemPrice += itemMap.get(itemObj[0])*itemObj[1];
        	        		}
        	    		}else{
        	    			break;
        	    		}
        	    		n++;
    	    		}
    	    		var personalRewardList = $("#personalRewardList").val();
	    			//console.log("====="+rewardList);
    	    		if(personalRewardList != "" && personalRewardList != undefined){
    	    			var personalRewardLists = personalRewardList.split(";");
    	        		for(var i = 0 ; i < personalRewardLists.length ; i++){
    	        			var itemObj = personalRewardLists[i].split(",");
    	        			itemPrice += itemMap.get(itemObj[0])*itemObj[1];
    	        		}
    	    		}
    	    		var con = confirm("${msg['activity.item.itemPrice']}"+itemPrice+"${msg['activity.item.isContinue']}");
    	    		if(con == false){
    	    			return;
    	    		}
    				$.post(base + "/activity/addSubmitMaterial", $("#activity_add").serialize(), function(data, status) {
    					alert(data.msg);
    					if (data.ok) {
    						activity_reload(actType);
    						showPage(pageNum, actType);
    					}				
    				});
    			}
    		} else {
    			alert("${msg['activity.validation.fail']}");
    		}
    	});
    	
    	$("#addTemplate").click(function() {
    		if ($('#activity_add').validationEngine('validate')) { //验证通过后才能add添加    			
    			var bTime = new Date($("#beginTime").val().replace(/\-/g, "\/"));
    			var eTime = new Date($("#endTime").val().replace(/\-/g, "\/"));
    			var curTime = new Date();
    			   			
    			if (eTime <= bTime) {
    				alert("${msg['activity.ebtime.alert']}");
    			} else if (eTime <= curTime) {
    				alert("${msg['activity.ectime.alert']}");
    			} else {
    				$("#isTemplate").val("1");
    				$.post(base + "/activity/addSubmitMaterial", $("#activity_add").serialize(), function(data, status) {
    					alert(data.msg);
    					if (data.ok) {
    						activity_template_reload(actType);
    					}
    				});  				
    			}    			    			
    		} else {
    			alert("${msg['activity.validation.fail']}");
    		}
    	});
    	
    	$("body").on("change", "#template", function() {    		
    		var createTime = $("#template").val();
    		if (createTime != "") {
    			$("#delTemplate").show();//将删除模板按钮显示
        		set_template(createTime, actType);
    		}else{
    			$("#delTemplate").hide();//将删除模板按钮隐藏
    		}    		
    	});
    });    
    </script>
    
</head>

<body>

	<table width="95%" height="95%" align="center">
		<tr><td valign="top"><br/>
		    <font style="white-space:nowrap" size=6><a href="${base}/activity/getPage?actType=0"><strong>${msg['activity.manager']}</strong></a></font>
		    <strong>——>${msg['activity.submitMaterial']}</strong><br/><br/>
        </td></tr>
        
        <tr><td>       
            <form id="activity_add" method="post" action="#">
            
            <table frame="box" width="95%" cellspacing="10">
                <tr align="left">
                <td colspan="2"><strong>${msg['activity.base.info']}</strong></td>
                <td colspan="2"  align="left">
                	模板名字：
                	<input type="text"  name="templateName" id="templateName" placeholder="自定义模板名字"/>
                	<input type="button" id="addTemplate" value="添加此模板"/>
                    <input style="display:none" id="isTemplate" name="isTemplate" type="text" value="1"/>选择模板<select id="template"></select>
                    <input type="button" style="display: none;" id="delTemplate" value="删除此模板"/>
                </td>
                </tr>
                           
                <tr align="left">
                <td>${msg['activity.name']}&nbsp;&nbsp;&nbsp;&nbsp;<input id="name" name="name" type="text" class="validate[required]"/></td>
                <td>${msg['activity.labelpos']}<input id="labelPosition" name="labelPosition" type="text" class="validate[required,custom[integer]]"/></td>
                <td>${msg['activity.labelorder']}<input id="labelOrder" name="labelOrder" type="text" class="validate[required,custom[integer]]"/></td>                        
                <td>${msg['activity.numlimit']}<input id="numLimit" name="numLimit" type="text" class="validate[required,custom[integer],min[0]]"/></td>
                </tr>
                
                <tr align="left">
                <td>${msg['activity.begintime']}<input name="beginTime" id="beginTime" type="text" class="datetimepicker validate[required]" readonly/></td>
                <td>${msg['activity.endtime']}<input name="endTime" id="endTime" type="text" class="datetimepicker validate[required]" readonly/></td>
                <td>${msg['activity.panelimageid']}<input id="panelImageId" name="panelImageId" type="text" class="validate[required,custom[integer],min[0]]"/></td>
                <td>${msg['activity.paneltext']}<input id="panelText" name="panelText" type="text" class="validate[required]"/></td>               
                </tr>              
                
                <tr align="left">
                <td>${msg['activity.submitMaterial.materialId']}<input name='materialId' id='materialId' type='text' list='materialIdList' class='validate[required]'/><datalist id='materialIdList'></datalist></td>
                </tr>
				
				<!-- 全服奖励 -->
              	<tr align="left">
                <td>${msg['activity.submitMaterial.allServerReward']}</td>
                </tr>
                <tr align="left">
                <td id="selectAllServerCondition" colspan="4">                 
                </td>
                </tr> 
                <tr>
                <td colspan="1"><input type="button" id="addAllServerCondition" value="+"/>
                <input type="button" id="deleteAllServerCondition" value="-"/></td>
                </tr>
                
                <!-- 个人提交保底奖励 -->
                <tr align="left">
                <td colspan="2">${msg['activity.submitMaterial.personalMinReward']}&nbsp;&nbsp;<input name="personalRewardList" id="personalRewardList" type="text" size="60" class="validate[required]" readonly/>
                <input type="button" id="addActivityItem" value="+"/><input type="button" id="deleActivityReward" value="-"/><input type="button" id="activityEmpty" value="${msg['activity.empty']}"/></td>
                </tr>
                
                <!-- 个人提交展示内容 -->
                <tr align="left">
                <td colspan="2">${msg['activity.submitMaterial.personalShowContent']}&nbsp;&nbsp;<input name="itemIds" id="itemIds" type="text" size="60" class="validate[required]" readonly/>
                <input type="button" id="addItemId" value="+"/><input type="button" id="deleItemId" value="-"/><input type="button" id="itemIdEmpty" value="${msg['activity.empty']}"/></td>
                <%--<input name="itemIds" id="itemIds" type="text" size="80" class="validate[required]" />
                 <span style="color:red;">${msg['activity.submitMaterial.showContentExplanation']}</span> --%>
                </tr>
                
                <!-- 个人提交奖励 -->
                <tr align="left">
                <td>${msg['activity.submitMaterial.personalReward']}</td>
                </tr>
                <tr align="left">
                <td id="selectPersonalCondition" colspan="4">                 
                </td>
                </tr> 
                <tr>
                <td colspan="1"><input type="button" id="addPersonalCondition" value="+"/>
                <input type="button" id="deletePersonalCondition" value="-"/></td>
                </tr>
                
                <tr align="left">
                <td colspan="2">${msg['activity.submitMaterial.beginTime']}<input name="subBeginTime" id="subBeginTime" type="text" class="validate[required]" /><span style="color:red;">*时间格式为hh:mm(小时:分钟)</span></td>
                <td colspan="2">${msg['activity.submitMaterial.endTime']}<input name="subEndTime" id="subEndTime" type="text" class="validate[required]" /><span style="color:red;">*时间格式为hh:mm(小时:分钟)</span></td>
                </tr>
                 
                <tr align="left">
                <td colspan="1">${msg['activity.activityType']}&nbsp;&nbsp;&nbsp;&nbsp;<select id="tag" name="tag" class="validate[required]"><option value="1">${msg['activity.holidayActivity']}</option></select></td>
                </tr>
                
                <!-- 帮助信息 -->
                <tr align="left">
                	<td colspan="4">${msg['activity.help']}<br/><textarea rows="10" cols="50" wrap="hard" id="help" name="help"></textarea></td>
                </tr>    
                <tr><td colspan="4"><input type="button" id="addSubmit" value="${msg['activity.add.submit']}"/></td></tr>
            </table>
                      
            </form>
        </td></tr>
        
        <tr><td>
            <div id="activity_list"></div>            
        </td></tr>
        
        <tr><td>
            <div id="pageIndex" style="float:left"></div>            
        </td></tr>        
        
	</table>

</body>

</html>