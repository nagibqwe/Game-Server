...
<head>
    ...
    <title>${msg['activity.exchange']}</title>
    	...
    	$("#addSubmit").click(function() {
    		if ($('#activity_add').validationEngine('validate')) {
    			...
    			if (eTime <= bTime) {
    				...
    				$("#isTemplate").val("0");
    				$.post(base + "/activity/addRechargeExtra", $("#activity_add").serialize(), function(data, status) {
    					alert(data.msg);
    					if (data.ok) {
...
<body>
	...
	<table width="95%" height="95%" align="center">
		<tr><td valign="top"><br/>
		    ...
		    <strong>——>${msg['activity.rechargeExtra']}</strong><br/><br/>
        </td></tr>
        ...
        <tr><td>       
            ...
            <table frame="box" width="95%" cellspacing="10">
                ...
                <td>${msg['activity.labelpos']}<input id="labelPosition" name="labelPosition" type="text" value="1" class="validate[required,custom[integer]]"/></td>
                <td>${msg['activity.labelorder']}<input id="labelOrder" name="labelOrder" type="text" value="1" class="validate[required,custom[integer]]"/></td>                        
                <td>${msg['activity.numlimit']}<input id="numLimit" name="numLimit" type="text" value="1" class="validate[required,custom[integer],min[0]]"/></td>
                </tr>
                ...
                <td>${msg['activity.panelimageid']}<input id="panelImageId" name="panelImageId" type="text" value="1" class="validate[required,custom[integer],min[0]]"/></td>
                <td>${msg['activity.paneltext']}<input id="panelText" name="panelText" type="text" class="validate[required]"/></td>               
                </tr> 
                
                <!--  
                <tr align="left">                              
                <td colspan="2">${msg['activity.reward']}&nbsp;&nbsp;&nbsp;&nbsp;<input name="rewardList" id="rewardList" type="text" size="60" class="validate[required]" readonly/>
                <input type="button" id="addItem" value="+"/><input type="button" id="deleReward" value="-"/><input type="button" id="empty" value="${msg['activity.empty']}"/></td>
                </tr> 
                -->                
                ...
                <tr align="left">
	                <td colspan="1">
	                	${msg['activity.rechargeExtra.rate']}：
	                	<input id="rate" name="rate" style="width:20px;" class="validate[required,custom[integer],min[1],max[100]]"> %
	                </td>                
                </tr>
