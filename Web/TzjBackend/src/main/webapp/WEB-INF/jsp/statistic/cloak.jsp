...
<title>${msg['jsp.cloak.title']}</title>
...
<script type="text/javascript">
	...
	function search() {
		...
		if (checkDate(startDate, endDate)) {
			...
			$
					.ajax({
						type : "POST",
						url : base + "/statistic/cloakStatistic",
						...
						success : function(data) {
							...
							var thead = $("<thead>");
							var htr = $("<tr>");
							var fields = [ "${msg['jsp.cloak.layer']}",
							        "${msg['jsp.cloak.upItemId']}",
									"${msg['jsp.cloak.upplayernumber']}",
									"${msg['jsp.cloak.clickcount']}",
									"${msg['jsp.cloak.totalnum']}" ];
	...
	function exportExcel(){
		$("#cloak table").eq(0).tableExport({type: 'excel', escape: 'false'});
	};
	function getItemName(itemId){
		switch (itemId) {
		case "101":
			return "${msg['jsp.cloak.gold']}";
		case "50027":
			return "${msg['jsp.cloak.pifengshengjieshi']}";
		default:
			return "${msg['jsp.cloak.unknown']}"+itemId;
		}
	};
</script>
</head>
	...
	<div class="container-fluid">
		<div>
			<form action="#" id="query_form" class="well form-inline">
				...
				<label class="label">${msg['jsp.cloak.chooseItem']}</label>
				<select name="itemId" class="span2">
					<option value="101" selected="selected">${msg['jsp.cloak.gold']}</option>
					<option value="50027">${msg['jsp.cloak.pifengshengjieshi']}</option>
					<option value="0">${msg['jsp.cloak.noitem']}</option>
				</select>
