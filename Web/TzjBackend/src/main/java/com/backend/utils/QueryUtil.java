...
public class QueryUtil {
     ...
     * 得到最终合服的dblog
     */
    public Dblog getFinalHeFuDB(int serverId) {
        ...
        if (dblog.getIsHeFu() == 0) {
            return dblog;
        }
        return getFinalHeFuDB(dblog.getHefuServerID());
    }
     ...
     * @param serverId 源服id
     */
    public List<Dblog> checkHeFu(int serverId) {
    ...
    private List<Dblog> getHeFuDBLogList(String idStr) {
        String sqlStr = "select * from $table where serverId in($ids) and isDeleted=0";
        Sql sql = Sqls.create(sqlStr);
        sql.vars().set("table", "t_dblog");
        sql.vars().set("ids", idStr);
        sql.setCallback(Sqls.callback.entities());
        sql.setEntity(dao.getEntity(Dblog.class));
        dao.execute(sql);
        return sql.getList(Dblog.class);
    }
     ...
     * @return 返回需要查询的服
     */
    public List<Dblog> checkHeFu(int serverId, Date sTime, Date eTime) {
        List<Dblog> dbList = new ArrayList<>();
        Dblog server = DbLogListManager.getInstance().getDblog(serverId);
        if (server == null) {
    ...
    private void checkHeFuServer(Dblog server, List<Dblog> list, Date sTime, Date eTime) {
        if (eTime.compareTo(new Date()) > 0) {
            eTime = new Date();
        }
        if (server.getIsHeFu() == 1) {
            List<Dblog> serverList = getHeFuServerList(server.getServerId());
            /*
              遍历所有此服和过的服务器日志库
              得到合服时间在查询时间段内的服务器日志库
              并加上合服时间在查询结束时间之后的第一个日志库（如果有的话）
             */
            Map<Integer, Dblog> serverKeyMap = new HashMap<>();
            for (Dblog serverDb : serverList) {
    ...
    private List<Dblog> getHeFuServerList(int serverId) {
        String sqlStr = "select * from $table where owerlist like $serverId";
        Sql sql = Sqls.create(sqlStr);
        sql.vars().set("table", "t_dblog");
    ...
    public Map<String, List<String>> getHefuTable(String serverId, String table, Date sTime, Date eTime) {
        ...
        List<Dblog> dblogList = checkHeFu(Integer.parseInt(serverId), sTime, eTime);
        Dao daoLog;
        for (Dblog dblog : dblogList) {
            ...
            Sql sql = Sqls.create(sqlStr);
            sql.vars().set("dbname", dblog.getDbname());
     ...
     * @return 实际存在的表名
     */
    public List<String> queryTables(Dblog dblog, String table) {
        SimpleDataSource dsLog = DbConfigUtil.getInstance().getSDS(dblog);
        Dao daoLog = new NutDao(dsLog);
        return queryTables(daoLog, dblog.getDbname(), table);
    }
     ...
     * @return 条数
     */
    public int queryCount(Dblog dblog, String sqlStr) {
        SimpleDataSource dsLog = DbConfigUtil.getInstance().getSDS(dblog);
        Dao daoLog = new NutDao(dsLog);
        Sql sql = Sqls.create(sqlStr);
        sql.setCallback((Connection conn, ResultSet rs, Sql sql1) -> {
    ...
    public int queryCount(Dblog dblog, String sqlStr, String table) {
        SimpleDataSource dsLog = DbConfigUtil.getInstance().getSDS(dblog);
        Dao daoLog = new NutDao(dsLog);
        Sql sql = Sqls.create(sqlStr);
        sql.vars().set("table", table);
     ...
     */
    public List<Map<String, Object>> query(Dblog dblog, String sqlStr) {
        SimpleDataSource dsLog = DbConfigUtil.getInstance().getSDS(dblog);
        Dao daoLog = new NutDao(dsLog);
     ...
     * @return list<map < string, obj>>
     */
    public List<Map<String, String>> query(Dblog dblog, String sqlStr, String table) {
     ...
     * @return list<map < string, obj>>
     */
    public List<Map<String, String>> query(Dblog dblog, String sqlStr, String table, Map<String, Object> paraMap) {
        SimpleDataSource dsLog = DbConfigUtil.getInstance().getSDS(dblog);
        Dao daoLog = new NutDao(dsLog);
     ...
     * @return List<?>
     */
    public List<?> queryWithEntity(Dblog dblog, String sqlStr, String table, Map<String, Object> paraMap, Class<?> resultClass) {
        SimpleDataSource dsLog = DbConfigUtil.getInstance().getSDS(dblog);
        Dao daoLog = new NutDao(dsLog);
     ...
     */
    @SuppressWarnings("unchecked")
    public Map<String, Map<String, String>> queryStat(Dblog dblog, String sqlStr, String key, Map<String, Object> paraMap) {
        SimpleDataSource dsLog = DbConfigUtil.getInstance().getSDS(dblog);
        Dao daoLog = new NutDao(dsLog);
