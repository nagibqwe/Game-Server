...
public class CrossManager {
    ...
    //世界服、跨服DBLOG
    private Map<Integer, Dblog> dblogMap = new ConcurrentHashMap<>();

    //世界服server
    private Map<String, List<Server>> psMap = new ConcurrentHashMap<>();
    //世界服db
    private Map<String, List<Dblog>> psDBMap = new ConcurrentHashMap<>();
    //跨服server
    private Map<String, List<Server>> fsMap = new ConcurrentHashMap<>();
    //跨服db
    private Map<String, List<Dblog>> fsDBMap = new ConcurrentHashMap<>();
    ...
    private void loadDB(int serverType, Map<String, List<Dblog>> map) {
        map.clear();
        List<String> groupList = getGroupInfo("t_dblog", serverType);
        List<Dblog> serverDBList;
        for (String groupName : groupList) {
    ...
    private List<Dblog> getSeverDBByGroupName(String groupName, int serverType) {
        List<Dblog> dblogList = dao.query(Dblog.class, Cnd.where("groupName", "=", groupName)
                .and("serverType", "=", serverType)
                .and("isDeleted", "=", 0)
                .asc("serverId"));
    ...
    public List<Dblog> getDBs(String groupName, int serverType) {
        switch (serverType) {
            case ServerType.Public:
    ...
    public Dblog getDB(int serverId) {
