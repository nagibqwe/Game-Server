    ...
    <form id="sendMailForm" method="post" class="form-horizontal well">
        ...
        <div class="control-group">
            ...
            <div class="controls">
                ...
                <input id="all" type="checkbox" class="checkbox inline" onclick="allServerMail()"/>全服邮件
            </div>
        </div>

        <div id="roleDiv" class="control-group">
            <label for="roleIds" class="control-label">${msg['kick.player']}</label>
            <div class="controls">
                <input id="roleIds" type="text" onblur="autocheckRoleIds()" name="roleIds" class="validate[required] span6" width="30px" placeholder="${msg['jsp.roleId.Info']}"/>
                <span id="autoCheckResult"></span>
                <br/>
                <input id="roleFile" type="file" onchange="fileSelected(this.files);"/>
                <label style="color: #00B83F">请上传txt格式文件，多个数据请使用换行来分割</label>
                <br/>
                <input type="button" class="btn btn-warning" value="${msg['jsp.roleId.check']}" onclick="checkRoleIds()"/>
