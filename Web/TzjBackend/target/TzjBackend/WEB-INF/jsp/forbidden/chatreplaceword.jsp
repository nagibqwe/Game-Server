...
<script type="text/javascript">
    ...
    function search() {
        ...
        $.ajax({
            url: base + "/forbidden/getChatReplaceWordList",
            data: {
                serverId: serverId
            },
            method: "post",
            ...
            success: function (data) {
                ...
                if (!data.ok) {
                    ...
                    html += "<tr><th>编号</th><th>关键字</th><th>替换内容</th><th>类型</th><th>操作</th></tr>";
                    for (var key in keyData) {
                        var typeInfo = keyData[key].type == 0 ? "关键字" : "一句话";
                        html += "<tr><td>" + keyData[key].id + "</td><td>" + keyData[key].word +"</td><td>" + keyData[key].replace+ "</td><td>" + typeInfo + "</td>" +
    ...
    function addShieldKeyWord() {
        ...
        var type = $("#type").val();
        $.ajax({
            url: base + "/forbidden/addChatReplaceWord",
            method: "post",
            dataType: "json",
            data: {
                serverId: serverId,
    ...
    function deleteShieldKeyWord(wordId) {
        $.ajax({
            url: base + "/forbidden/deleteChatReplaceWord",
            method: "post",
            dataType: "json",
            data: {
    ...
    <div id="addModal" class="modal hide fade">
        ...
        <div class="modal-body">
            <form id="add_keyword" class="form form-horizontal">
                ...
                <div class="control-group">
                    <label for="replace" class="control-label">替换内容</label>
                    <div class="controls">
                        <input id="replace" type='text' placeholder='替换内容'/>
                    </div>
                </div>
            </form>
        </div>
