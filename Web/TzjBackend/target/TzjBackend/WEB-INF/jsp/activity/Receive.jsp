...
<head>
    ...
    <title>${msg['activity.receive']}</title>
    ...
    function getCondition(index) {    		
   		...
   		switch(condition) {
   		case "2":
   			detail_condition ="<input id=\"level"+index+"\" name=\"level"+index+"\" type='text' size='5' class='validate[required,custom[integer],min[0]]'/>${msg['activity.receive.condition.lv']}";    			
   			break;
   		case "9":
   			detail_condition = "${msg['activity.receive.condition.vip.lv']}<input id=\"vipLevel"+index+"\" name=\"vipLevel"+index+"\" type='text' size='8' class='validate[required,custom[integer],min[1],max[12]]'/>";
   			break;
   		case "10":
   			detail_condition = "${msg['activity.receive.condition.equip.tostar']}<input id=\"equipTotalStar"+index+"\" name=\"equipTotalStar"+index+"\" type='text' size='8' class='validate[required,custom[integer],min[0]]'/>";
   			break;
   		case "11":
   			detail_condition = "${msg['activity.receive.condition.equip.id']}<input id=\"equipID"+index+"\" name=\"equipID"+index+"\" type='text' size='8' class='validate[required,custom[integer],min[0]]'/>\
   				                &nbsp;${msg['activity.receive.condition.equip.id.star']}<input id=\"needStar1"+index+"\" name=\"needStar1"+index+"\" type='text' size='8' class='validate[required,custom[integer],min[0]]'/>";
   			break;
   		case "12":
   			detail_condition = "${msg['activity.receive.condition.equip.pos']}<input id=\"equipPos"+index+"\" name=\"equipPos"+index+"\" type='text' size='8' class='validate[required,custom[integer],min[0],max[10]]'/>\
   				                &nbsp; ${msg['activity.receive.condition.equip.id.star']}<input id=\"needStar2"+index+"\" name=\"needStar2"+index+"\" type='text' size='8' class='validate[required,custom[integer],min[0]]'/>";
   			break;
   		case "13":
   			detail_condition = "${msg['activity.receive.condition.equip.tostrenlv']}<input id=\"totalStrengthLevel"+index+"\" name=\"totalStrengthLevel"+index+"\" type='text' size='8' class='validate[required,custom[integer],min[0]]'/>";
   			break;    			
   		default:
   			break;
   		}
    	...
    	$("#addSubmit").click(function() {
    		if ($('#activity_add').validationEngine('validate')) {
    			...
    			if (eTime <= bTime) {
    				...
    				$.post(base + "/activity/addReceive", $("#activity_add").serialize(), function(data, status) {
    					alert(data.msg);
    					if (data.ok) {
...
<body> 
	...
	<table width="95%" height="95%" align="center">
		<tr><td valign="top"><br/>
		    ...
		    <strong>——>${msg['activity.receive']}</strong><br/><br/>
        </td></tr>
        ...
        <tr><td>       
            ...
            <table frame="box" width="95%" cellspacing="10">
                ...
                <tr align="left">                
                <td colspan="4">${msg['activity.isOnceAgain']}&nbsp;&nbsp;&nbsp;<select id="isOnceAgain" name="isOnceAgain" class="validate[required]">
                <option value="1">${msg['activity.onceAgain']}</option><option value="0">${msg['activity.noOnceAgain']}</option></select></td>
                </tr>
