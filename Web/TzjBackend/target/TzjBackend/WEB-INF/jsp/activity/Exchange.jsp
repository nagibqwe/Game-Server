...
<head>
    ...
    function needItemEmpty(index) {
		$("#needItem"+index).val("");
	}
	
	function deleNeedItem(index) {
		var tempItem = $("#needItem"+index).val();
		if (tempItem != null && tempItem != "") {
			...
			$("#needItem"+index).val(strItem);
		}
	}
	
	function needItemShowBox(index) {
		$.ajax({
		    		...
		    		afterShow:function() {
		    			...
		    			$("#add").click(function() {
		    				if ($('#add_item').validationEngine('validate')) { //物品信息验证通过后组装物品信息为字串填到奖励物品栏中
		    					...
		    					var itemInfo = $("#needItem"+index).val();		    					
		    					if (itemInfo != '') {
		    						itemInfo += ";";
		    					}
    	...
    	$("#addSubmit").click(function() {
    		if ($('#activity_add').validationEngine('validate')) {
    			...
    			if (eTime <= bTime) {
    				...
    				$.post(base + "/activity/addExchange", $("#activity_add").serialize(), function(data, status) {
    					alert(data.msg);
    					if (data.ok) {
...
<body>
	...
	<table width="95%" height="95%" align="center">
		<tr><td valign="top"><br/>
		    ...
		    <strong>——>${msg['activity.exchange']}</strong><br/><br/>
        </td></tr>
        ...
        <tr><td>       
            ...
            <table frame="box" width="95%" cellspacing="10">
                ...
                <tr align="left">
                <td colspan="4"><strong>${msg['activity.reward.condition']}</strong>
                <span style="color:red;">(${msg['activity.exchange.notice']})</span></td>
                </tr>
                ...
                <tr>
                <td colspan="1">${msg['activity.loop.type']}<select id="loopType" name="loopType" class="validate[required]"><option value="1">${msg['activity.loop.day']}</option>
