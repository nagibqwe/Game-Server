    ...
    $(function() {      	    	
    	...
    	$("#addSubmit").click(function() {
    		if ($('#activity_add').validationEngine('validate')) {    			
    			...
    			var sbTime = new Date($("#dayStartTime").val().replace(/\-/g, "\/"));
    			var seTime = new Date($("#dayEndTime").val().replace(/\-/g, "\/"));
    			...
    			if (eTime <= bTime) {
    				...
    				alert("${msg['activity.top.ebtime.alert']}");
    			} else if (seTime <= curTime) {
    				alert("${msg['activity.top.ectime.alert']}");
    			} else {
    				$("#actType").val(actType);
    				$("#isTemplate").val("0");
    				var itemPrice = 0;
    				...
    				$.post(base + "/activity/addTopReward", $("#activity_add").serialize(), function(data, status) {
    					alert(data.msg);
    					if (data.ok) {
...
<body>
	...
	<table width="95%" height="95%" align="center">
		<tr><td valign="top"><br/>
		    ...
		    <strong>——>${activityTitle}</strong><br/><br/>
        </td></tr>
