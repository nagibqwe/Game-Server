...
<head>
    ...
    function dropTypeChange(index) {
    	var type = $("#dropType"+index).val();
    	console.log("dropType=" + type);       	
    	if (type == 1) {
    		$("#mapIdinput"+index).css("display","none");//隐藏div
    		$("#monsterIdinput"+index).css("display","none");//隐藏div
    	} else if (type == 2) {
    		$("#mapIdinput"+index).css("display","inline");//显示div
    		$("#monsterIdinput"+index).css("display","none");//隐藏div
    	} else if (type == 3) {
    		$("#mapIdinput"+index).css("display","none");//隐藏div
    		$("#monsterIdinput"+index).css("display","inline");//显示div
    	}
	};
	
	function noticeChange(index) {
    	var isNotice = $("#isNotice"+index).val();      	
    	if (isNotice == 1) {
    		$("#contentinput"+index).css("display","inline");//显示div
    	} else {
    		$("#contentinput"+index).css("display","none");//隐藏div
    	}
	};
    $(function() {      	    	
    	...
    	$("#addSubmit").click(function() {
    		if ($('#activity_add').validationEngine('validate')) {
    			...
    			if (eTime <= bTime) {
    				...
    				$.post(base + "/activity/addDrop", $("#activity_add").serialize(), function(data, status) {
    					alert(data.msg);
    					if (data.ok) {
...
<body>
	...
	<table width="95%" height="95%" align="center">
		<tr><td valign="top"><br/>
		    ...
		    <strong>——>${msg['activity.drop']}</strong><br/><br/>
        </td></tr>
        ...
        <tr><td>       
            ...
            <table frame="box" width="95%" cellspacing="10">
                ...
                <!--  
                <tr align="left">               
                <td colspan="4">${msg['activity.description']}&nbsp;&nbsp;&nbsp;&nbsp;<input name="description" type="text" size="60" class="validate[required]"/></td>               
                </tr>  
                -->               
