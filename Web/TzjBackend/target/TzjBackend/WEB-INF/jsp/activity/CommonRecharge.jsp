    ...
    $(function() {    	   	
    	...
    	$("#addSubmit").click(function() {
    		...
    		if ($('#activity_add').validationEngine('validate')) {
    			...
    			var reBTime = new Date($("#reBeginTime").val().replace(/\-/g, "\/"));
    			var reETime = new Date($("#reEndTime").val().replace(/\-/g, "\/"));
    			...
    			if (eTime <= bTime) {
    			...
    			} else if (reETime <= reBTime) {
    				alert("${msg['activity.rrtime.alert']}");
    			} else {
    				$("#isTemplate").val("0");
    				var itemPrice = 0;
    				var n = 1;
    				...
    				$.post(base + "/activity/addCommonRecharge", $("#activity_add").serialize(), function(data, status) {
    					alert(data.msg);
    					if (data.ok) {
    	...
    	$("body").on("change", "#template", function() {    		
    		...
    		if (createTime != "") {
    			$("#delTemplate").show();//将模板显示
        		set_template(createTime, actType);
    		}else{
	    		$("#templateName").val("");
    			$("#delTemplate").hide();//将模板隐藏
    		}    		
    	});
    });    
...
<body>
	...
	<table width="95%" height="95%" align="center">
		<tr><td valign="top"><br/>
		    ...
		    <strong>——>${msg['activity.recharge']}:${msg['activity.recharge.common']}</strong><br/><br/>
        </td></tr>
        ...
        <tr><td>       
            ...
            <table frame="box" width="95%" cellspacing="10">
                ...
                <td colspan="2"  align="left">
                	模板名字：
                	<input type="text" id="templateName" name="templateName" placeholder="自定义模板名字"/>
                	<input type="button" id="addTemplate" value="添加此模板"/>
                    <input style="display:none" id="isTemplate" name="isTemplate" type="text" value="1"/>
                    	选择模板<select id="template"></select>
                ...
                <tr align="left">
                <td>${msg['activity.recharge.begintime']}<input name="reBeginTime" id="reBeginTime" type="text" class="datetimepicker validate[required]" readonly/></td>
                <td>${msg['activity.recharge.endtime']}<input name="reEndTime" id="reEndTime" type="text" class="datetimepicker validate[required]" readonly/></td>
                </tr>
