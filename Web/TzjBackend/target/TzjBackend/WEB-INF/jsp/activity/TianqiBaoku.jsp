...
<head>
	...
	function rewardEmpty(index) {
		$("#conditionList"+index).val("");
	}
	
	function deleItem(index) {
		var tempItem = $("#conditionList"+index).val();
		if (tempItem != null && tempItem != "") {
			...
			$("#conditionList"+index).val(strItem);
		}
	}
	
	function itemShowBoxTianQi(index) {
		$.ajax({
			url : base + "/global/getItemInfo",
			dataType : "json",
			success : function(jsonData) {
				var rewardTemp =  $("#conditionList"+index).val();
				if(rewardTemp != ''){
					alert("${msg['activity.cloudBuy.alert']}");
					return ;
				}
		    		...
		    		afterShow:function() {
		    			...
		    			$("#add").click(function() {
		    				if ($('#add_item').validationEngine('validate')) { //物品信息验证通过后组装物品信息为字串填到奖励物品栏中
		    					...
		    					var itemInfo = $("#conditionList"+index).val();		    					
		    					if (itemInfo != '') {
		    						itemInfo += ";";
		    					}
		    					...
		    					$("#conditionList"+index).val(itemInfo);
    	...
    	$("#addSubmit").click(function() {
    		if ($('#activity_add').validationEngine('validate')) {    			
    			...
    			if (eTime <= bTime) {
    	    		...
    	    		var n = 1;
    	    		while(n){
    	    			var conditionList = $("#conditionList"+n).val();
    	    			//console.log("====="+rewardList);
        	    		if(conditionList != "" && conditionList != undefined){
        	    			var conditionLists = conditionList.split(";");
        	        		for(var i = 0 ; i < conditionLists.length ; i++){
        	        			var itemObj = conditionLists[i].split(",");
    	    		...
    	    		var m = 1;
    	    		var comProbability = 0;
    	    		while(m){
    	    			var probabilityList = $("#probability"+m).val();
    	    			//console.log("====="+probabilityList);
        	    		if(probabilityList != "" && probabilityList != undefined){
        	        		comProbability += probabilityList*1;
        	    		}else{
        	    			break;
        	    		}
        	    		m++;
    	    		...
    	    		var gold = 0;
    	    		for(var i=0;i<16;i++){
    	    			if($("#selectType"+i).val()==2){
    	    				var goldList = $("#conditionList"+i).val();
            	    		if(goldList != "" && goldList != undefined){
            	    			gold += goldList*1;
            	    		}
    	    			}
    	    		}
    	    		if(gold!=100){
    	    			alert(gold);
    	    			alert("${msg['activity.item.goldCount']}"+100);
    	    			return ;
    	    		}
    	    		//console.log("====="+comProbability);
    	    		var con = confirm("${msg['activity.item.isContinue']}");
    	    		if(con == false){
    	    			return;
    	    		}
    	    		if(comProbability != 100000){
    	    			alert("${msg['activity.cloudBuy.comProbability']}"+100000);
    	    			return ;
    	    		}
    				$.post(base + "/activity/addTianQiBaoku", $("#activity_add").serialize(), function(data, status) {
    					alert(data.msg);
    					if (data.ok) {
	...
	function selectType(index){
		...
		if(selectType==1){
			$("#conditionList"+index).attr("readOnly",true);
			$("#empty"+index).show()
			$("#addItem"+index).show()
			$("#reduceItem"+index).show()
		}
		if(selectType==2){
			$("#conditionList"+index).attr("readOnly",false);
			$("#empty"+index).hide()
			$("#addItem"+index).hide()
			$("#reduceItem"+index).hide()
		}
	}
    </script>
...
<body>
	...
	<table width="95%" height="95%" align="center">
		<tr><td valign="top"><br/>
		    ...
		    <strong>——>${msg['activity.cloudBuy']}</strong><br/><br/>
        </td></tr>
        ...
        <tr><td>       
            ...
            <table frame="box" width="95%" cellspacing="10">
                ...
                <tr align="left">
                <td colspan="4">转盘内容</td>
                </tr>
                <% for(int i = 1 ; i <= 16 ; i++){%>
	                <tr align="left">
		                <td colspan="4">
		                	<div>${msg['activity.baoku.index']}<%=i%>&nbsp;&nbsp;&nbsp;
		                	<select name="selectType<%=i%>" id="selectType<%=i%>" onchange="selectType(<%=i%>)"><option value="1">${msg['activity.baoku.item']}</option><option value="2">${msg['activity.baoku.gold']}</option></select>
		                	<input name="conditionList<%=i%>" id="conditionList<%=i%>" type="text" size="60" class="validate[required]" readonly/>
		     	        	<input type="button" id="addItem<%=i%>" value="+" onclick="itemShowBoxTianQi(<%=i%>);"/>
		     	        	<input type="button" id="reduceItem<%=i%>" value="-" onclick="deleItem(<%=i%>);"/>
		     	        	<input type="button" id="empty<%=i%>" onclick="rewardEmpty(<%=i%>);" value="${msg['activity.empty']}"/>
		     	        	&nbsp;&nbsp;&nbsp;${msg['activity.cloudBuy.probability']}
		     	        	<input id ="probability<%=i%>" name="probability<%=i%>" type=text size=5 class="validate[required,custom[integer],min[1]]"/>
		     	        	&nbsp;&nbsp;&nbsp;${msg['activity.baoku.notice']}
		     	        	<input id ="notice<%=i%>" name="notice<%=i%>" type=text size=5 class="validate[required,custom[integer]]"/>
		     	        	&nbsp;&nbsp;&nbsp;${msg['activity.baoku.effect']}
		     	        	<input id ="effect<%=i%>" name="effect<%=i%>" type=text size=5 class="validate[required]"/>
		     	        	</div>
		                </td>  
	                </tr>
                <% }%>
                ...
                <tr>
	                <td colspan="4">
	                	${msg['activity.baoku.joinCounts']}：&nbsp;&nbsp;<input name="joinCounts" id="joinCounts" type="text" class="validate[required]"/>
		                
	                </td>
                </tr>
                <tr align="left">
                <td colspan="4"><strong>${msg['activity.baoku.change']}</strong></td>
                </tr>
                <tr align="left">
