...
<head>
    ...
    <title>${msg['activity.dayReceive']}</title>
    ...
    function dataParse(actId) {
    	$.post(base + "/activity/QueryActivity", {actId:actId}, function(data, status) {
            if (data.ok) {
            	var activity = data.data;
            	//$("#name").val(activity.name);
            	//$("#labelPosition").val(activity.labelPosition);
            	//$("#labelOrder").val(activity.labelOrder);
            	$("#numLimit").val(activity.numLimit);
            	//$("#beginTime").val(activity.beginTime);
            	//$("#endTime").val(activity.endTime);
            	$("#panelImageId").val(activity.panelImageId);
            	$("#panelText").val(activity.panelText);
            	$("#description").val(activity.description);
            	$("#rewardList").val(activity.rewardList);
            	var conArr = activity.conditionList.split("_");
            	$("#itemId").val(conArr[0]);
            	$("#dayStartTime").val(conArr[1]);
            	$("#dayEndTime").val(conArr[2]);
            	$("#receiveNum").val(conArr[3]);
            } else {
            	alert(data.msg);
            }
        });
    ...
    $(function() {      	    	
    	...
    	$.ajax({
			...
			success : function(jsonData) {
				...
				$("#giftItemIdList").html(item_option);
			}
		});
    	
    	/* $("#choice").click(function(){
    		$.post(base + "/activity/query", {actType:9}, function(dataResult, status) {
    			if(!status) {
    				alert("当前网络出问题了！" + status);
    				return;
    			}
    			
				var boxy_html = "<form id='activitylist' method='post' action='#'><div>${msg['activity.dayReceive.selectAct']}<select id='activityId' class='validate[required]'><option value='' selected='selected'>----${msg['activity.daySend']}----</option>";
				for (var i = 0; i < dataResult.list.length; i++) {
					var activity = dataResult.list[i];
				
					//console.log(item.itemId + "," + item.itemName + "," + item.itemMax);
					boxy_html += "<option value='" + activity.id + "'>" +activity.name+"_"+activity.conditionList +"</option>";
				}
				
				boxy_html += "</select>*\
					<br/><br/><input id='activityId_add' type='button' value='${msg['activity.item.add']}'/></div></form>";
		    	...
		    	var boxy = new Boxy(boxy_html,
		        {
		    		...
		    		afterShow:function() {
		    			$("#activitylist").validationEngine();
		    			$("#activityId_add").click(function() {
		    				if ($('#activitylist').validationEngine('validate')) { //物品信息验证通过后组装物品信息为字串填到奖励物品栏中
		    					var activityId = $("#activityId").val();
								dataParse(activityId);		    					
    	...
    	$("#addSubmit").click(function() {
    		if ($('#activity_add').validationEngine('validate')) {
    			...
    			if (eTime <= bTime) {
    				...
    				$.post(base + "/activity/addDayGet", $("#activity_add").serialize(), function(data, status) {
    					alert(data.msg);
    					if (data.ok) {
...
<body>
	...
	<table width="95%" height="95%" align="center">
		<tr><td valign="top"><br/>
		    ...
		    <strong>——>${msg['activity.dayReceive']}</strong><br/><br/>
        </td></tr>
        ...
        <tr><td>       
            ...
            <table frame="box" width="95%" cellspacing="10">
                ...
                <%-- <td colspan="1"><input type="button" id="choice" value="${msg['activity.dayReceive.selectAct']}"></input></td> --%>
                </tr>
                ...
                <tr align="left">                
                <td>${msg['activity.sendReceive.itemId']}<input name='giftItemId' id='giftItemId' type='text' list='giftItemIdList' class='validate[required]'/><datalist id='giftItemIdList'></datalist>
                </td> 
                </tr>               
