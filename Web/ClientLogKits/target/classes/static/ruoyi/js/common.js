    ...
    editColumn: function() {
    	...
    	for (var dataIndex = 0; dataIndex < count; dataIndex++) {
    	    ...
    	    for (var i = 0; i < columns.length; i++) {
                ...
                var checkbox = $(columns[i]).find(':checkbox');
    	        var key = dataColumns[i].field;
    	        if ($.common.isNotEmpty(inputValue.val())) {
                    if (checkbox.length > 0 && i > 0){//排除每行最前面的checkbox框
                        var checkboxValueArr = [];
                        for (var j = 0; j < checkbox.length; j++){
                            var serverLabelName = $("input:checkbox[name='servers["+dataIndex+"].serverLabel']");
                            var isChecked = serverLabelName.eq(j).prop("checked");
                            if (isChecked){
                                checkboxValueArr.push(j);
                            }
                            obj[key] = checkboxValueArr.toString();
                        }
                    }else{
