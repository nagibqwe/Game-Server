...
var loginServerNameInfo = new Map();
var loginServerIPInfo = new Map();
...
function deleteServerExtra(serverId) {
    ...
    if (serverId == 0){
        ...
        $.modal.confirm("确定删除吗？", function() {
            serverIdsArr.push(parseInt(serverId));
            serverIdsArr.sort();
            serverIds.push(parseInt(serverId));
            reloadServers(serverIdsArr);
            $("#bootstrap-table").bootstrapTable('remove', {field: 'serverId', values: serverIds});
        });
    }
...
//初始化下拉框取值范围
function getRange() {
    var select = $("#showCronExpression");
    select.empty();
    //以秒为单位
    for (var i = 1; i < 60; i++) {
        var option = $("<option>").val(i).text(i+"秒");
        select.append(option);
    }
    //以分为单位
    for (var i = 1; i < 60; i++) {
        var option = $("<option>").val(i*60).text(i+"分钟");
        select.append(option);
    }
}
//格式化执行周期
function formatterCronExpression(value, row, index) {
    var showValue = "";
    if (parseInt(value) > 59) {
        showValue = value / 60 + "分钟";
    } else {
        showValue = value + "秒";
    }
    return showValue;
...
//加载所有登录服信息
function loadLoginServerInfo() {
    $.ajax({
        url: ctx + "serverListConfig/loginServer/selectLoginServers",
        method: "post",
        dataType: "json",
        data: {
        },
        ...
        success: function (data) {
            if (data.length > 0){
                for (var i=0;i<data.length;i++){
                    $("#loginServers").append("<option value='" + data[i].id + "'>" + data[i].loginServerName + "("+data[i].loginServerIp+")</option>")
                    loginServerNameInfo.set(data[i].id + "", data[i].loginServerName);
                    loginServerIPInfo.set(data[i].id + "", data[i].loginServerIp);
                }
            }
        }
    });
...
//加载服务器列表中已选择的登录服显示
function loadShowLoginServers(serverListId) {
    $.ajax({
        url: ctx + "serverListConfig/loginServer/selectShowLoginServers",
        method: "post",
        dataType: "json",
        data: {
            "serverListId": serverListId
        ...
        success: function (data) {
            ...
            if (data.length > 0){
                var loginServers=[];
                for (var i=0;i<data.length;i++){
                    loginServers.push(data[i].id);
                    html+=data[i].loginServerName+"("+data[i].loginServerIp+")"+"&nbsp;&nbsp;";
                }
                $('#loginServers').selectpicker('val', loginServers);//设置关联渠道下拉框选中对应的已有渠道
            }
            $("#showLoginServers").html(html);
        }
    });
}

//登录服下拉框改变选择时,文本显示追加选中的登录服
$("#loginServers").on('changed.bs.select',function () {
    var loginServers = $('#loginServers').val();
    $("input[name='loginServers']").val($("#loginServers").val());
    console.log("loginServers:"+loginServers);
    var html="";
    if (loginServers == null){
        $('#showLoginServers').html(html);
    } else {
        for (var i=0;i<loginServers.length;i++){
            html+=loginServerNameInfo.get(loginServers[i])+"("+loginServerIPInfo.get(loginServers[i])+")|";
        }
        $('#showLoginServers').html(html);
    }
});