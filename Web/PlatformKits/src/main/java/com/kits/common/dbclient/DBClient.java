...
//	public DBClient(String jdbcUrl, String userName, String password, int dbWriterPartNum, int dbCacheWriterPartNum, int maxConnections, int minConnections) throws SQLException, ClassNotFoundException {
...
//	}
	...
	public DBClient(DBServerConfig dbServerConfig) throws SQLException, ClassNotFoundException {
		this.connectionPool = new DBConnectionPool(dbServerConfig.getJdbcUrl(), dbServerConfig.getUserName(), dbServerConfig.getPassword(),dbServerConfig.getDataSourceName(),dbServerConfig.isUseConnectionPool());
	}
	 ...
	 * 注:1.基础表专用 2.T为Integer,Long,String等基础数据类型时 clazz传null
	 * */
	public <T> List<T> selectList(String sql,Object[] params, Class<T> clazz) throws SQLException {
		...
		try {
			...
			QueryRunner qr = new QueryRunner();
			if (clazz == null) {
				results = qr.query(conn, sql,params, new ColumnListHandler<T>());
			} else {
				ResultSetHandler<List<T>> rsh = new BeanListHandler<T>(clazz);
				results = qr.query(conn, sql, params,rsh);
			}
	...
	public <T> List<T> selectList(String sql,Object[] params) {
		...
		try {
			...
			QueryRunner qr = new QueryRunner();
			results = (List<T>) qr.query(conn, sql, params,new MapListHandler());
	...
	public <T> List<T> selectList2(String sql,Object... params) {
		Connection conn = null;
		List<T> results = null;
		try {
			conn = getConnection();
