...
LOGPACKET=Log-`date +%Y-%m-%d-%H-%M-%S`
...
#获取后台地址
getHeartHttp() {
	egrep "heartHttp" $GAMEPATH/config/server-config.xml | awk -F '\"' '{print $2}'
}
...
unPackServer(){
	...
	#cd $WORLDPATH
	if [ -f worldserver.tar.gz ];then
		tar -zxvf worldserver.tar.gz
		echo "world 解压完毕"
	fi
...
manager(){
	...
	case $1 in
		...
		*)
		echo "错误的命令输入！！！"
		echo "请输入控制命令： start or stop or restart"
		;;
	esac
