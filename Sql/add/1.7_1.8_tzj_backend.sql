...
ALTER TABLE `t_server` ADD COLUMN `registerNum` INT DEFAULT 0 NOT NULL COMMENT '服务器注册人数' AFTER `heartTime`; 

ALTER TABLE `t_recharge` ADD COLUMN `rechargeTotalGold` INT NULL COMMENT '充值累积数量' AFTER `rechargeNumber`;
ALTER TABLE `t_recharge` ADD COLUMN `rechargeVipExp` INT NULL COMMENT '充值VIP经验' AFTER `rechargeTotalGold`;

ALTER TABLE `t_recharge_item` ADD COLUMN `goods_pay_type` INT DEFAULT 0 NULL COMMENT '支付渠道（第三方支付）' AFTER `goods_pay_channel`;
ALTER TABLE `t_recharge_item` ADD COLUMN `isTotalRecharge` INT DEFAULT 0 NULL COMMENT '是否计入到游戏累充活动' AFTER `goods_extra_reward_limit`;
ALTER TABLE `t_recharge_item` ADD COLUMN `totalVipPower` INT DEFAULT 0 NULL COMMENT '是否增加VIP经验' AFTER `isTotalRecharge`;


ALTER TABLE `t_activity` ADD COLUMN `isOpenServer` INT(32) DEFAULT 0 NULL COMMENT '是否是新服活动' AFTER `autoSend`; 
ALTER TABLE `t_activity_template` ADD COLUMN `isOpenServer` INT(32) DEFAULT 0 NULL COMMENT '是否是新服活动' AFTER `autoSend`;
