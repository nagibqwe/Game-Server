...
ALTER TABLE `t_activity` DROP COLUMN `templateName`, ADD COLUMN `configData` TEXT NULL COMMENT '配置参数，用于GM后台反解析' AFTER `cover`; 
...
CREATE TABLE `game_info` (
  `id` varchar(128) DEFAULT NULL COMMENT '游戏ID',
...
CREATE TABLE `t_recharge_item` (
  ...
  `systemCfgId` int(32) DEFAULT NULL COMMENT '游戏内部配置ID',
  `goodsDesc` varchar(128) DEFAULT NULL COMMENT '商品名字描述（主要用于BI后台数据）',
  `dealType` int(32) DEFAULT NULL COMMENT '交易类型',
  `type` int(32) DEFAULT NULL COMMENT '充值类型',
  `subType` int(32) DEFAULT NULL COMMENT '充值子类型',
  `goodsLimit` int(32) DEFAULT NULL COMMENT '充值次数（当前轮每个挡位对应充值的次数)',
  `icon` int(32) DEFAULT NULL COMMENT '显示的图标的ID',
  `money` varchar(128) DEFAULT NULL COMMENT '充值档位对应消耗的真实货币',
  `reward` varchar(128) DEFAULT NULL COMMENT '充值奖励',
  `multiple` varchar(128) DEFAULT NULL COMMENT '充值奖励倍数',
  `extraReward` varchar(128) DEFAULT NULL COMMENT '额外奖励',
  `extraRewardCount` int(32) DEFAULT NULL COMMENT '额外奖励次数',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
