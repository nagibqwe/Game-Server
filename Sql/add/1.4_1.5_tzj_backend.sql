...
ALTER TABLE `t_dblog` ADD COLUMN `serverOpenTime` VARCHAR(128) NULL COMMENT '开服时间' AFTER `isDeleted`;


ALTER TABLE `game_info` CHANGE `id` `gameId` int(11) NULL COMMENT '修改时间'; 
ALTER TABLE `game_info` ADD COLUMN `autoServerId` INT DEFAULT '0' COMMENT '自动开服ID' AFTER `rechargeSecretkey`; 

ALTER TABLE `t_recharge_item` CHANGE `dealType` `goods_pay_channel` VARCHAR(128) DEFAULT '' NULL COMMENT '渠道名称'; 
ALTER TABLE `t_recharge_item` CHANGE `money` `money` VARCHAR(1000) DEFAULT '' NULL COMMENT '充值档位对应消耗的真实货币'; 
ALTER TABLE `t_recharge_item` ADD COLUMN moneyPoint varchar(500) DEFAULT '' COMMENT '充值计费点' AFTER money;
ALTER TABLE `t_recharge_item` ADD COLUMN `showMoneyIcon` VARCHAR(128) DEFAULT '' NULL COMMENT '默认显示货币' AFTER `moneyPoint`; 


ALTER TABLE `t_server` ADD COLUMN `serverOpenTime` VARCHAR(128) NULL COMMENT '开服时间' AFTER `isShow`; 
ALTER TABLE `t_server` ADD COLUMN `openState` int(32) DEFAULT NULL COMMENT '服务器状态 0:备服状态 1:开服状态' AFTER `serverOpenTime`; 
ALTER TABLE `t_server` ADD COLUMN `heartTime` varchar(128) DEFAULT NULL COMMENT '服务器最新心跳时间' AFTER `openState`; 
ALTER TABLE `t_server` CHANGE `openState` `openState` INT(32) DEFAULT 0 NOT NULL COMMENT '服务器状态 0:备服状态 1:开服状态'; 


ALTER TABLE `t_activity` ADD COLUMN `openServerRecordOffsetBegin` int(32) DEFAULT NULL COMMENT '记录距离开服多少天' AFTER `endTime`; 
ALTER TABLE `t_activity` ADD COLUMN `openServerRecordOffset` int(32) DEFAULT NULL COMMENT '活动记录持续天数' AFTER `openServerRecordOffsetBegin`; 
ALTER TABLE `t_activity` ADD COLUMN `startRecordTime` varchar(128) DEFAULT NULL COMMENT '开始记录时间' AFTER `openServerRecordOffset`; 
ALTER TABLE `t_activity` ADD COLUMN `endRecordTime` varchar(128) DEFAULT NULL COMMENT '结束记录时间' AFTER `startRecordTime`; 
ALTER TABLE `t_activity` ADD COLUMN `autoSend` int(32) DEFAULT '0' COMMENT '开服自动发布活动标识，0：否，1：是' AFTER `isDeleted`; 


ALTER TABLE `t_activity_template` ADD COLUMN `openServerRecordOffsetBegin` int(32) DEFAULT NULL COMMENT '记录距离开服多少天' AFTER `endTime`; 
ALTER TABLE `t_activity_template` ADD COLUMN `openServerRecordOffset` int(32) DEFAULT NULL COMMENT '活动记录持续天数' AFTER `openServerRecordOffsetBegin`; 
ALTER TABLE `t_activity_template` ADD COLUMN `startRecordTime` varchar(128) DEFAULT NULL COMMENT '开始记录时间' AFTER `openServerRecordOffset`; 
ALTER TABLE `t_activity_template` ADD COLUMN `endRecordTime` varchar(128) DEFAULT NULL COMMENT '结束记录时间' AFTER `startRecordTime`; 
ALTER TABLE `t_activity_template` ADD COLUMN `autoSend` int(32) DEFAULT '0' COMMENT '开服自动发布活动标识，0：否，1：是' AFTER `endRecordTime`; 


ALTER TABLE `t_backend_log` CHANGE `content` `content` LONGTEXT NULL COMMENT '操作内容'; 
...
(251, '充值配置', 3, 12, NULL, 'rechargeItem/recharge_item', '', 0),
