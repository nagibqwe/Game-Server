...
public class ClassUtil {
    ...
    public static Class<?> javaCodeToObject(String name, String code, String classpath, String classFilePath) throws Exception {
        ...
        try (StandardJavaFileManager fileManager = compiler.getStandardFileManager(diagnostics, null, null)) {
            ...
            // 重新加载类需要用单独的类加载器九 零一 起玩 www.90  175.com
        ...
        if (reload) {
            String singleName = name.substring(name.lastIndexOf(".") + 1);
            String packagePath = name.substring(0, name.lastIndexOf("."));
            File file = new File(classFilePath.replace(".", "/") + "/" + packagePath.replace(".", "/"));
            File[] files = file.listFiles(new FileFilter() {
                @Override
                public boolean accept(File file) {
                    //TODO 获取所有 匿名类 和内部类
                    return file.getName().contains(singleName + "$") && file.getName().endsWith(".class");
                }
            });
            ...
            for (File f :list) {
                ...
                //TODO 加载 匿名类 和内部类
                loader.loadScriptClass(classFilePath, packagePath + "." + className);
            }
    ...
    public static <T> void getClasssFromJarFile(JarFile jarFile, String filePaht, Set<Class<T>> classes, Class<T> parentClass) throws IOException, ClassNotFoundException {
        ...
        for (JarEntry entry : jarEntryList) {
            ...
            if (parentClass.equals(loadClass)) {
                continue;
            }
            if (parentClass.isAnnotation()) {
                @SuppressWarnings("unchecked")
                Class<? extends Annotation> annotation = (Class<? extends Annotation>) parentClass;
